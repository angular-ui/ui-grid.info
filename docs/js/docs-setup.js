NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope above actual address age aggregate aggregation all allow allowing allows alpha always an and angular any app appropriate are as asc at attempt average avg balance based because been being below body breaks btn btn-success but button buttons by calculate can capability cellfilter change class cleanly clear column columndef columndefs columns company complex consideration controller count counting counts create css currency custom data deep deeper default definitions dependency desc describe development direction distribution does doesn don done each easily edit effect enablefiltering enhancement entity example excel expand expandall expandallrows external fake feature fields files filtered filtering find first for format front function functionality functions gender get gets going grid grid1 gridapi gridoptions gridtestutils group grouped groupheader grouping groupingindent groupingrowheaderwidth groupingsuppressaggregationtext grouppriority has header height however html id if impacts important in include included indent indented index indication insertion internal is isn it js json larger length level leverage leverages limit list look main mainctrl make marked max mean means min module more moved moving name names need needed needs ng-click ng-controller nganimate ngtouch nicer non-grouped not notable notice notify number objects obtain of on only onregisterapi optionally options or order out outstandings overview people permit permitted pinning pivot pixels pleasing present probably programmatically provides providing proxy re refer rendercontainers replace require returned reversed row rownum rows rtl scenario second see set should similar slice so some sort sorting spec specific start state still success sums suppress suppressremovesort sure table testing text that the themselves then therefore this to together togglefirstrow togglerow togglerowgroupingstate togglesecondrow true tutorial type ui ui-grid ui-grid-grouping ui-grid-pinning uigridgroupingconstants under ungrouped unit using usually value values var visiblerowcache visual visually want watch ways we well what when whether which wider width with work xxxx yet you your"
    },
    {
      "section": "tutorial",
      "id": "210_selection",
