NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "ColumnDef",
      "type": "object",
      "keywords": "aggregating aggregation also and api are ascending at available be but called can class column columndef columndefs columns constants count defined determine do enable enablegrouping expect feature file for format generally getgrouping given grid gridoptions group grouping grouppriorities grouppriority high if in include irrespective is less ll look max min next no not object of on or order priority put re renumber sequential set should sort starts sum than that the them then there these they this time to true types ui ui-grid uigridgroupingconstants undefined use using values we what will xxxx"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "object",
      "keywords": "aggregation api are available constant constants count grid grouping in max min module note object of particular sum ui uigridgroupingconstants"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope about above actual add adding address age aggregate aggregation aggregations all allow allowing allows alpha always an and angular any app appropriate are as asc at attempt average balance based be been being body break btn btn-success but button buttons by can capability change check class cleaner cleanly column columndef columndefs columns company complex consideration controller count counting counts create css custom data dates deep deeper default definitions dependency desc describe development direction distribution does doesn done each edit effect enablefiltering enhancement entity example excel expand expandall expandallrows external fake feature fields files filtered filtering find first for front function functionality functions gender get gets grid grid1 gridapi gridoptions gridtestutils group grouped groupheader grouping groupingindent groupingrowheaderwidth grouppriority has header height however html id if impacts important in include included indent indented index initialised internal is isn it item items js json larger length level leverage leverages like likely limit list look main mainctrl make marked max maybe mean menu min module more moved moving naive name names need needed needs ng-click ng-controller nganimate ngtouch nicer non-grouped not notable notice notify number objects obtain of on only onregisterapi optionally options or order out outstandings overview people permit permitted pinning pivot pixels pleasing present probably programmatically provides providing proxy quite re refactor remove rendercontainers repetitive require returned reversed row rownum rows rtl scenario second set should similar smell so some somehow sort sorting spec specific start state still stored success sums suppressremovesort sure table testing that the themselves then therefore they things this to together togglefirstrow togglerow togglerowgroupingstate togglesecondrow too total true tutorial type types ui ui-grid ui-grid-grouping ui-grid-pinning uigridgroupingconstants under ungrouped unit using value values var visiblerowcache visually watch ways we well when whether which wider width with work yet you your"
    },
    {
      "section": "tutorial",
      "id": "210_selection",
