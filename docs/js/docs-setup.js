NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "Grid",
      "type": "function",
      "keywords": "add added additional all already alter an and any api appended are array as asc ascending assign assigntypes automatically be behavior being block browser build buildcolumndefsfromdata buildcolumns buildstyles by called calls cell chance class col coldef column columnbuilder columnbuilders columndefinition columndefs columns columnsprocessor container contains count created createleftcontainer createrightcontainer creates currently data defined definitions desc descending direction directions directive doesn each either element etc exclude exist existing expected false field first for from function further getcoldef getcolumn getcolumnsorting getrow given grid gridcolumn gridoptions gridrow handlewindowresize has having id identified if in instance into is it iterate its left load long main maintain map methods modified modifyrows name needed newrawdata not object objects of on one only optional optionally options or pass per populates precompilecelltemplates precompiles process processes processor processrowbuilders promise properties property prototype provided reference register registercolumnbuilder registercolumnsprocessor registered registerrowbuilder registerrowsprocessor registerstylecomputation remove removerowsprocessor removes render renderable reset resetcolumnsorting resetting resizes resolved resources return returns right row rowbuilder rowbuilders rowentity rowequality rows set sort sortcolumn sorted sorting state stylecomputation supplies templates that the this through to triggered type types ui unsorted updated uses using value viewmodel when which will window with"
    },
    {
      "section": "api",
