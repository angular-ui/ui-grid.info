NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "columnDef",
      "type": "object",
      "keywords": "$scope about action active add address adds against aggregation aggregationhidelabel alert algorithm allowing alongside although an and angular any api append apply are array asc auto backward be bind binding blargh boolean by call can cell cellclass cellfilter cellnav celltemplate city class clicked col colrenderindex column columndef columndefs compatibility complex condition configuration contain content context controls currently custom data date default defaults defining definition definitions desc determine direction disablecolumnmenu disablehiding disables displayed displayname div docs each either element enabled enablesorting ends ends_with evaluate evaluates example expression expressions false feature field field1 field2 field3 fields filter filtering filters focus footer for from function functionality generated getfulladdress grid gridoptions gridutil guess guessing guesstype have header headercellfilter headercelltemplate hides hiding icon id if in individual is it item items just knows label like list many maximum maxwidth menu menuitems menus method minimum minwidth multiple must name normal not number object of omitted on one only option options or outer parameters pass percentage placeholder present prevents property provided receive refer removesort return returns rowrenderindex satisfaction scope see selected service set sets setting settings should show shown single so sort sorting sortingalgorithm source specific specify specifying starts starts_with string supported suppressremovesort takes template term text that the then this title to too translate true tutorial type typically ui ui-grid ui-grid-icon-info-circled uigridconstants use used user using value values visible when whether which widgets width will wish with within work would xxx you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "306 Exporting Data With Custom UI",
      "type": "overview",
