NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "function",
      "keywords": "address against aggregate aggregation aggregationhidelabel algorithm allocated although an and angular any api are aren array backward based baz be belongs bind binding body bound by calculated can city class classname coldef column columns compatibility complex condition container converts current data deciding default defined definition display displayname docs each element elements ends ends_with evaluates expression expressions false field filter filters foo for function generated getaggregationtext getaggregationvalue getcolclass getcolclassdefinition getfulladdress getrendercontainer gets grid have header hidecolumn hides i18n if in includes including index input instead into is label like lookup makes methods must name needed ngmodel normal not object of omitted on one options or parameters placeholder position prefixdot present property prototype provided reference render represents return returns see setting should showcolumn shown sorting sortingalgorithm source specifically starts starts_with state string takes term text th that the then they this to true type ui uigridconstants use using value viewmodel visible whether will wish with you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "columnDef",
      "type": "object",
      "keywords": "$scope about action active add address adds against aggregation aggregationhidelabel alert algorithm allowing allows alongside although an and angular any api append apply are array asc auto backward be bind binding blargh boolean by call can cell cellclass cellfilter cellnav celltemplate city class clicked col colrenderindex column columndef columndefs compatibility complex condition configuration contain content context controls custom data date default defaults defining definition definitions desc determine direction disablecolumnmenu disablehiding disables displayed displayname div docs each either element enable enabled enablefiltering enablesorting ends ends_with evaluate evaluates example expression expressions false feature field field1 field2 field3 fields filter filtering filters focus footer for function functionality generated getfulladdress grid gridoptions gridutil guess guessing guesstype have header headercellfilter headercelltemplate hide hides icon id if in individual is it item items just knows label like list many maximum maxwidth menu menuitems menus method minimum minwidth multiple must name normal not number object of omitted on one only option options or outer parameters pass percentage placeholder present property provided receive refer removesort return returns rowrenderindex satisfaction scope see service set sets setting settings should show shown single so sort sorting sortingalgorithm source specific specify specifying starts starts_with string supported suppressremovesort takes template term text that the then this title to too translate true tutorial type typically ui ui-grid ui-grid-icon-info-circled uigridconstants use used user using value values visible when whether which widgets width will wish with within work would xxx you your"
    },
    {
      "section": "api",
