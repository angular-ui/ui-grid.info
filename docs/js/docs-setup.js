NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope able adds all allows and angular api app are as at automatically available be being bold both bower buttons can class color columndefs com company controller css csv currentpage custom-csv-link-location data directive docdefinition download element enable enablegridmenu enableselectall example explorer export exported exporter exportercsvfilename exportercsvlinkelement exporterpdfcustomformatter exporterpdfdefaultstyle exporterpdffooter exporterpdfheader exporterpdfmaxgridwidth exporterpdforientation exporterpdfpagesize exporterpdftableheaderstyle exporterpdftablestyle exporting false feature field fontsize footerstyle for format found from function gender get grid gridapi gridoption gridoptions have header headerstyle height html https if in include index install installed internet is issues italics items js json letter main mainctrl menu module must my myfile name native need ng-controller nganimate ngtouch not note of on only onregisterapi open opening option options or other overview pagecount pdf pdfmake portrait queryselectorall rather red repo return rows selected selection show style styles success tabs text than that the then there this through to tostring true tutorial ui ui-grid ui-grid-exporter ui-grid-selection use using var visible want we width will with you your"
    },
    {
      "section": "tutorial",
