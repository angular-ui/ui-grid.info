NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "GridApi",
      "type": "function",
      "keywords": "$log $scope ability acol add again all allow allowed and api array be binds callback callbackfn cellnav class components defaults dereg disables disabling enables event eventname eventnameone eventnametwo eventobjectmap events execute executes feature featurename features for format from function functions funtion get given grid gridapi gridoptions in inside it listen listener listenerfunc listenerfuncs listenerfunctions listeners log map method methodname methodnameone methodnametwo methods must name names navigate navigation new objectmap of oldrowcol on onregisterapi or other our owns prepended provides public raise raises register registerevent registereventsfromobject registermethod registermethodsfromobject registers remove returns same scrollto simple something specified still suppress suppressevents that the then these this thisarg to ui use used var via were while will you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "all allow also always and api are at available be being bulk called can cause changed checkbox class column conjunction create ctrlkey custom different either enable enablefootertotalselected enablerowheaderselection enablerowselection enableselectall enableselectionbatchevent entire event false feature fired footer for from grid gridapi gridoptions header if in instead is items modifierkeystomultiselect multiple multiselect must nounselect number object of only option or prevent requires row rows rowselectionchanged select selectall selected selection selectionrowheader selectionrowheaderwidth selectrow separate set setting shiftkey showfooter shows single the then these this times to top total true ui ui-grid unselect unselected used using via when width will with works you"
    },
    {
      "section": "api",
