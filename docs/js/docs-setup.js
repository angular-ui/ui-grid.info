NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "function",
      "keywords": "$delegate $provide $resource $scope $translate acting action active add added adding adds addtogridmenu adjusted advantage again aggregates all allow allowing allows also altering alternative always am an and angular angular-translate angularjs any api app application appscope appscopeprovider aquamarine are array as assign assigned associate associated at attribute attributes auto-generating automatically available avoid away background-color backwards be becomes been behaviour being better binding boolean bottom boxes but by callback called can case cases center changes check class col colcontainer coldef column columndef columndefs columnfooterheight columns columnvirtualizationthreshold common compare compared compatibility complex conditions config configure contain content context controller controls converts copy core could current custom customisation data decorator default defaults define defined described developer directly disable disabled display displayed displayname displays div do documented does doesn dom down dynamic each either element elements empty enablecolumnmenus enabled enablefiltering enablehorizontalscrollbar enableminheightcheck enablerowhashing enablesorting enableverticalscrollbar end enough entire entities entitya entityb equality event events example excesscolumns excessrows exclude excludeproperties executing existing exists external extra factory false features field field1 field2 fields file filter filterchange filtering fired fires first flat flatentityaccess fnone follow footer footer_template footertemplate for format from function functions further generate get getrowidentity getsafetext github goes grid gridapi gridmenu gridmenucustomitems gridmenushowhidecolumns gridmenutitlefilter gridoptions grids gridutil handle handles has have header header_template headerrowheight headers headertemplate height helps hide hit hits horizontal horizontalscrollthreshold how html i18nservice id identical identifying identity if ignore ignored implemented implying improvement in include included individual infinite infinitescrolldown infinitescrollpercentage infinitescrollup information initialize initialized initoptions inline instantiated integer interact internal internationalization into inverse is it item items large least list listen logic look maintain management maxvisiblecolumncount may menu menus method middle minimum minimumcolumnsize minrowstoshow modified modify more most multiple must mydata name names necessary needed needmoredata needmoredatatop never newly nextuid ng-click ng-repeat nice no not note null number numbers object objects of often on once one only onregisterapi operate option options or out outside over overlaid overridden page parent pass percentage performance pixels place places point precompiled present pretty prevents process programatically promise properties property propery provide provided provides providing query rather refer reference related remaining remove removefromgridmenu render rendered renderedcolumns renderingcomplete replace request requested required resize resulting return returns row row_template rowequality rowheight rowidentity rows rowtemplate scope scroll scrollbar scrollbars scrolling scrollthreshold scrollthrottle second section see selectallrows selection server set sets setting settings should show showcolumnfooter showgridfooter showheader shown significant simplify situation smaller smoothness some sort sorter sorting source sourced specify speed speeding standard starting state static still string stripped structure style sub-objects supported suppresses tailor take takes tall template terms text-align than that the their then they this throttle time title to todo tojson top total track trigger triggered true turn tutorial ui ui-grid ui-grid-bottom-panel ui-grid-cell ui-grid-header ui-grid-top-panel uigrid uigridconstants unique uniquely up updated use used useexternalfiltering useexternalsorting user users uses using value values var vastly vertical viewport virtualization virtualizationthreshold visibility visible want way we were what when whenever where whether which while widgets will with within working would x_ you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "PublicAPI",
      "type": "object",
      "keywords": "additional any api are call can data dataloaded direction don downward downwards end events example feature fire flag for function grid have in indicate infinite infinite_scroll infinitescroll loaded more no nomoredatabottom nomoredatatop object of or pages public reached requested scroll see set so that the there this to tutorial ui upward upwards usage ve we when won you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "service",
      "keywords": "account actually addition adjust adjustinfinitescrollposition adjustscroll an and anyway api are args assuming assumptions back based be been better bottom but by called case clean closer come comes data determines do down downwards else end even event events experience features fires first for from function get good gotten grid handlescroll has have if in infinite infinitescroll informed initializegrid into it jump last ll loaddata loaded look make making may mean-time method methods more much need needloadmoredata needloadmoredatatop neither new not now of off old on once or pages percentage position public re really register request requires row rows run scoll scroll scrolldirection scrolled scrolling scrolls service set should smoother so someone somewhere still takes that the then there these they things this through time to top trigger ui until up upwards want wants we whenever whether which while will working would you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope added adjust age allows already also and angular api app appearance are array as assume at attempt available away back basically be because beginning behaviour bottom but by call can change class columndefs comes concat container continuous controller css data dataloaded default direction down downwards each either enabled end error event events example exist feature finally firstpage firstpage-- for from function further get getdatadown getdataup getpage give grid gridapi gridoptions has have height hit html id ideally if in index infinite infinitescroll infinitescrolldown infinitescrollpercentage infinitescrollup instead is it js json jumpy largely lastpage lazy length listen little load loaded long lot main mainctrl may module more move must name need needloadmoredata needloadmoredatatop needmoredata needmoredatatop never newdata newly ng-controller ngtouch no normal not notify null number of on once onregisterapi optionally or overview page pages parent percentage percentages present propagate propagating push raise reach reached real record records reflect request requests res respond retrieved return rows scroll scrolled scrolling see set should smooth so specify start starts still stop success suppress tell that the their then there they this through time to told top touchpad trigger triggering true tutorial ui ui-grid ui-grid-infinite-scroll up up-front us user var was way we when which width will won work would you your zero"
    },
    {
      "section": "tutorial",
