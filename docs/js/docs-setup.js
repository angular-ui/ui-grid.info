NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "api are at available base be but button by children class control create don dynamically each enable enabletreeview entire even expand extra false feature for grid gridoptions header icon if incremented indent indents is level load make more need no node number object of pixels planning pleasing programatically provides re row set show showtreeexpandnochildren showtreeviewrowheader single states the there these this to tree treeview treeviewindent treeviewrowheaderbasewidth true ui ui-grid use using view visually wider width will you youll"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope adding additional address age algorithm all allow allows alpha an and angular anywhere app are arrangement array arrives as at automatically balance be body break btn btn-success but button buttons by calculates can cellfilter child children class columndefs company controller could counts create css currency current data deeper defaults delay describe development did display distribution doesn down dynamic dynamically each enablefiltering enablesorting entity even event example expand expandall expandallrows expanded false feature female files first for from function gender general get gets given grid grid1 gridapi griddable gridoptions grids gridtestutils guerrero have header height hourglass however html icon id if in include included increase incrementally indented index initial into is it js json know larger leaves length level levels listen ll load loaded loading look lopez main mainctrl make male means module most move name new ng-click ng-controller nganimate ngtouch nice nicer no node nodeloaded nodes not notable number of on only onregisterapi options or out outstandings overview page people perhaps permit pixels point positional property rather re render rendercontainers replace require retrieve right row rowexpanded rownum rows scenario second sense server set setting shows showtreeexpandnochildren simulate slice sort sorting spec specifying splice start state still structure success tell than that the then there they this though to togglefirstrow togglerow togglerowtreeviewstate togglesecondrow tree treeview treeviewindent treeviewrowheaderwidth true tutorial type ui ui-grid ui-grid-tree-view uigridtreeviewconstants under us user using values var very view visiblerowcache was watch we were when whenever whether which whilst whole width will wish within would you your"
    },
    {
      "section": "tutorial",
