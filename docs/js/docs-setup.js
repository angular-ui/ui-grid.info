NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "$elm $translate a4 accordingly add adding adjusted alignment allowing allows already an and angular-translate any api applied apply are array as automatically available be before behaviour being black bold browsers but button buttons by call callback called calling can cannot case changed changes character class codes col color column columns com comes complete complex containing content context controls convert csv current currentpage custom data date decodes decodestatus default defaults definition display displayname displaynames docdefinition document don download downloaded each element example export exported exporter exportercsvcolumnseparator exportercsvfilename exportercsvlinkelement exporterfieldcallback exporterheaderfilter exporterheaderfilterusename exporterheadertemplate exporterlinklabel exporterlinktemplate exportermenucsv exportermenulabel exportermenupdf exporterpdfcustomformatter exporterpdfdefaultstyle exporterpdffooter exporterpdfheader exporterpdfmaxgridwidth exporterpdforientation exporterpdfpagesize exporterpdftableheaderstyle exporterpdftablelayout exporterpdftablestyle exportersuppresscolumns exportersuppressmenu exporting exports false feature field filename filters fixed fontsize footer footerstyle for format from function go grid gridcol gridlines gridoptions gridrow had have header headerfilter https if implying in instant instead internationalisation into is it items js landscape layout leads left letter like link list mandatory margin massaged massaging match maxium means menu method more must my name names native need needed no not note null numbers object of on once only or orientation other our output own page pagecount paper part pass passed pdf pdfmake placed portrait present promise provide provides providing raw ready require resulting return returns right roll routine row saving scaled section selection selectionrowheader separator set should shouldn show shown simple size sizes so some specific status style styles styling supported suppressbutton suppressed synchronous system table tableheader tablelayout takes template text that the their then these this through to tostring true ui ui-grid updated use useful user using usually valid value ve we were when which width will with work you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$filter $http $scope address age agemax agemin aggregation aggregationhidelabel aggregationtype aggregationtypes also an and angular app are as avg background-color be below bound btn btn-success but can cellfilter class col column columndef columndefs columns controller copy count create css custom customcelltemplate data date default demonstrated describe display displayed displayname each enablefiltering enterfilterincolumn enum evaluate ever example expect expectfootercellvaluematch expectfootercolumncount expectheadercolumncount field filter footer footerbutton footercell footercellfilter footercelltemplate footers footertemplate for foreach from function functions get getaggregationvalue getmaxdatestring grid grid1 gridoptions gridtestutils have height html id if in index inject injector is it js json label logic main mainctrl max maxdate maxdateformattedstring maxfootercell min mmm module name need ng-click ng-controller ngtouch not on option or order override overview own parse pass point recalculate red refer registered require return row rows scenario set should show showfooter showfooterrow six source spec specific starting street style success sum supported supports template the then third this to total true tutorial ui ui-grid ui-grid-cell-contents ui-grid-footer uigridconstants use value values var visible what which white width will wish with without you your yyyy"
    },
    {
      "section": "tutorial",
      "id": "106_binding",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$apply $http $scope __columndef about active address addressformatter adjusted aftercelledit age all allow allows also alternative although an and angular angular-translate any app apply array arrows as available aware backing bar be begin_cell_edit boolean but by called can cancel_cell_edit cat cell celleditablecondition cellfilter cellnav cellscope changed checkbox city class code coldef column columndef columndefs columns compiled contain content contents controller controls css custom data datatype date default definitions depend determine directive disable displayname dog double-click dropdown edit editable editablecelltemplate editdropdownfilter editdropdownidlabel editdropdownoptionsarray editdropdownrowentityoptionsarraypath editdropdownvaluelabel edited editing editor editors element else enable enablecelledit enablecelleditonfocus enabled end_cell_edit ends enter entity esc events example f2 false feature female field fields filter fish focus following foo for function gender genderhash get grid gridapi gridoptions hamster happens has having height html id if in include including index inline input instead invoke invoked is isactive it js json key label labels lastcelledited left length likely logic main mainctrl male mapgender mode module msg must name need needs new newvalue next ng-controller ngtouch non-navigable not note number numeric object of oldvalue on only onregisterapi option options options__ or other overview pet point populated property provide provided purposes registered reprocess respectively return returns right row rowentity rowrenderindex rows scope set setting should similar simple so soon sorting specified start state street string success tab tags templatecache templates text than that the this those through to translate true tutorial two type types typing ui ui-grid ui-grid-edit uigrideditor url use used using valid value values var variable via were when width will with within without would xxx xxxxxxx you your yyyy-mm-dd zip"
    },
    {
      "section": "tutorial",
      "id": "202_cellnav",
