NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "RowSorter",
      "type": "object",
      "keywords": "algorithms and api applying appropriate be class column columns criteria default for grid guessing in including itself look mechanisms method object provides rows rowsorter sort sorted sorting sorts the to types ui which"
    },
    {
      "section": "api",
      ...
      "shortName": "PublicApi",
      "type": "object",
      "keywords": "active all always an and any api array as at be both but by calculations call called calls can changed circumstances clearrowinvisible clears columns complete completes core criteria currently data emits end event feature features filter filtering for from function functional get getcolumnsorting getting getvisiblerows grid gridapi gridcolumns gridoptions gridrow handlewindowresize has have if in included instance invisible is it just make manually method more necessary normal normally not note object obtain of on one onregisterapi or order other otherwise output override parameters picked present priority problem promise provides public refresh refreshrows render rendered rendering renderingcomplete resize resolved returned returns row rowentity rows rowsvisiblechanged same screen set setrowinvisible sets size so some sort sortchanged sortcolumns sorted sorting sorts still stopping that the them then there this time timing to todo trigger ui up users using visibility visible want watch way we when which will window with within without would you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $q $resource $scope active address addressformatter after aftercelledit again age all almost also an and angular another any app application are array as at attempts avoid back basic be because been boolean but button by callback callbacks called calling can causing cell celleditablecondition cellfilter cellnav city class clean clicks columndefs columns commences complete completing configurable controller create css currently data date deals defer delay destroyed dirty displayed displayname doesn during each easily edit editable edited editing edits either else enablecelledit enabled error errored errors event example exceptions experience extends extent fake faked false feature field fields filter flag flushdirtyrows flushes for four from function gender generate get getdirtyrows geterrorrows give go grey greyed grid gridapi gridoptions gridrows has hasn have height html id if in index input invisible is isactive isdirty iserror issaving js json last length local locking made main mainctrl male method methods might module more moving must name navigation new ng-controller no non-editable normally not number object occassionally occurs of off on one only onregisterapi operate operation optionally or other otherwise out overview page perspective point pressing promise properties provided ready red regime registered reject rejected request resolve resolved return returned returns row rowedit rowentity rowrenderindex rows same save saved saverow savetimer saving scope seconds seek server set setfocus setsavepromise shown similar since situations so some spreadsheet state states status still street success successfully sufficient support tabs take that the then they this time timeout timer to trigger triggered tutorial type ui ui-grid ui-grid-cellnav ui-grid-edit ui-grid-row-edit updated upon use used user using var wait were when whenever where which whichever whilst width will wish with within yet you yyyy-mm-dd zip"
    },
    {
      "section": "tutorial",
      "id": "210_selection",
