NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$filter $http $scope address age agemax agemin aggregation aggregationhidelabel aggregationtype aggregationtypes also an and angular app are avg background-color be below bound btn btn-success but can cellfilter class col column columndef columndefs columns controller count create css custom customcelltemplate data date ddescribe display displayed displayname each enablefiltering enterfilterincolumn enum evaluate ever example expect expectfootercellvaluematch expectfootercolumncount expectheadercolumncount field filter footer footerbutton footercell footercellfilter footercelltemplate footers for foreach function functions get getaggregationvalue getmaxdatestring grid grid1 gridoptions gridtestutils have height html id if in index inject injector is it js json label logic main mainctrl max maxdate maxdateformattedstring maxfootercell min mmm module name need ng-click ng-controller ngtouch on option or order overview own parse pass recalculate red refer registered require return row rows scenario set should show showfooter showfooterrow six spec specific street style success sum supported supports template the then third to total true tutorial ui ui-grid ui-grid-cell-contents uigridconstants use value values var visible what which white width will wish with without you your yyyy"
    },
    {
      "section": "tutorial",
      "id": "106_binding",
