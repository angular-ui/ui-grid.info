NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "function",
      "keywords": "$delegate $provide $resource $scope acting action active add added adding adds addtogridmenu adjusted advantage again all allow allowing allows also altering alternative am an and angular angularjs any api application aquamarine are array as at attribute attributes auto-generating automatically available background-color backwards be becomes been behaviour being better binding boolean boxes but by can case cases center changes class col colcontainer coldef column columndef columndefs columns common compare compared compatibility complex conditions config configure contain content context controller controls core could current custom customisation data decorator default defaultoptions defaults define defined described developer disable disabled display displayname displays div do documented does doesn dynamic each either elements empty enable enablecolumnmenu enabled enablefiltering enablerowhashing enablescrollbars enablesorting end entire entitya entityb equality event events example exclude excludeproperties executing existing exists external factory false features field field1 field2 file filter filterchange filtering fired fires first fnone follow footer footer_template footertemplate for format from function functions generate get getexternalscopes getrowidentity github grid gridmenu gridmenucustomitems gridmenushowhidecolumns gridoptions grids gridutil handle handles has have header header_template headers headertemplate height hide hideheader hit horizontal how html id identifying identity if ignore ignored implemented in include included individual infinite infinitescrollpercentage information inline integer internal into inverse is it item items large list listen logic look maintain management may menu menus method minimum minrowstoshow modified modify more most must mydata name names necessary new nextuid ng-click ng-repeat no not note null number numbers object objects of often on one only operate option options or out over overlaid overridden percentage performance place point precompiled pretty prevents process properties property provide provided provides providing query rather refer remove removefromgridmenu rendered renderedcolumns replace request requested required resulting return returns row row_template rowequality rowidentity rows rowtemplate scroll scrollbar second section server set setting settings should show shown simplify some sort sorter sorting source sourced specify speeding standard starting state static still string stripped style suppresses tailor take template text-align than that the their then they this to tojson track true tutorial ui ui-grid ui-grid-bottom-panel ui-grid-cell ui-grid-header ui-grid-top-panel uigrid unique uniquely up updated use used useexternalfiltering useexternalsorting users uses using value var vastly vertical visibility visible want way we what when whenever where whether which while widgets will with within working x_ you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "directive",
      "keywords": "$columnelement $scope again allows already an angular animate api app as ask asks assumption below break by calculate calls can column controller default directive displayed do doesn element elements expressions function grid hidden hide hidemenu hides ie8 if in interpolate is it mainctrl make menu method module ng-controller on once place position repositions right show showmenu shown shows size style the then this to true ui ui-grid-menu us var want we whilst width will"
    },
    {
      "section": "api",
      "id": "ui.grid.directive:uiGridColumnMenu",
    ...
    },
    {
      ...
      "type": "service",
      "keywords": "$scope added addmenuitem adds addtogridmenu all allowing always an and any api appropriate are array as associated be been by can change changed column columndef columndefs columns combination contains context core decides default dynamically each expects features find for from getmenuitems grid gridcol gridcolumn gridmenu gridmenucustomitems gridmenuscope gridmenuservice has have hide importantly in included individual information initialize is it item items ll menu method methods most need not object of on one only onto operate options our passed property provided re recalculated reference refresh register removefromgridmenu scope second service sets show showhidecolumns shown that the these they this through to toggle togglecolumnvisibility toggles two typically ui up upon us using visibility visible want we when whenever which with within working"
    },
    {
      "section": "api",
      "id": "ui.grid.i18n",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope accessing action add additive alert alert-warning alexander alisha all allows an and angular another any app app2 application are as asc at be below beryl but by can class claudine click clickcolumnmenuremovesort clickcolumnmenusortasc clickcolumnmenusortdesc clickheadercell clicking column columndefs columns company controller core css data def default defining definitions desc describe direction disabled don else enable enablesorting ethel example existing expectcellvaluematch expected expectheadercellvaluematch expectheadercolumncount false feature female field first flag foley for from fry function gender get gets grid grid1 grid2 gridapi gridoptions gridtestutils has have header headers height html id if in include index index2 inital-sort initial is it js json key last level main main2 mainctrl male menu menus module more multiple must myers name neal need ng-controller nganimate no not null nulls on one onregisterapi option options order original other overview page parsons pick price priority property rather remove removes replacing require required return reverse rice rows says scenario second see set setting shift shift-click shift-clicking shiftclickheadercell should slide so sort sorted sorthandlenulls sorting sortingalgorithm sorts span8 spec state sub-property success than that the then third three to true tutorial two ui ui-grid unless unsorted up use using values var velma visible want way what when which width will woods you your yvonne"
    },
    {
      "section": "tutorial",
      "id": "103_filtering",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope above absolute access action actions additional adds addtogridmenu also angular app are at available be box-shadow but by can class clickgridmenuitem column columns company controller core css csv customised data default describe dynamic element enabled enablegridmenu expectheadercellvaluematch expectheadercolumncount expectvisiblegridmenuitems export exporter exportermenucsv exportermenucsvexport false feature floats function gender gender_hide gender_show get gives grid grid1 gridapi gridmenucustomitems gridoption gridoptions gridtestutils have header height hidden hide html icon id if in index is it item items js json least main mainctrl menu module my-custom-menu name ng-controller nganimate none of one only onregisterapi option overview padding position require right rotate rotated row rows scenario selected selection set setting settings should show some spec success suppresses the then there this three through title to toggleclass top transform true tutorial ui ui-grid ui-grid-exporter ui-grid-menu ui-grid-menu-inner ui-grid-selection var visible which width"
    },
    {
      "section": "tutorial",
