NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope accessing action add additive after alert alert-warning alexander algorithm alisha all allows although an and angular another anthony any api app application are around as asc asynchronously at automatically aware away based be behind below beryl btn btn-success but button by calling calls can change changed chosen class claudine click clickcolumnmenuremovesort clickcolumnmenusortasc clickcolumnmenusortdesc clickheadercell clicking clicks code col-xs-12 column columndef columndefs columns company controller core css cycle data datachange date decide def default defining definitions demonstrate desc describe direction disabled documented don edit element else enable enablesorting ethel example existing expect expectcellvaluematch expected expectheadercellvaluematch expectheadercolumncount expects expectvisiblecolumnmenuitems explicitly false feature female field first flag foley for from fry function gender get gets gonzales grid grid1 grid1api grid2 grid2api gridapi gridoptions1 gridoptions2 gridtestutils guess has have header headers height html id if in include index initial is it javascript joyner js json key last let level live load main mainctrl male menu menus module more move multiple must myers name neal need ng-click ng-controller nganimate ngtouch no not notify notifydatachange null nulls number numberstr object of often on one onregisterapi option options order original other overview page parsons pick prevent price priority property provide rather recalculated redisplay remove removed removes removing replacing require required return reverse rice rows says scenario scenes second see set setting shift shift-click shift-clicking shiftclickheadercell should slide so sort sorted sorthandlenulls sorting sortingalgorithm sorts spec state string sub-property success suppressremovesort take than that the then third this three through to toggle togglegender toggles top true tutorial twice two type types ui ui-grid uigridconstants undisplay unless unsorted up use user using valid values var velma visible want way we what when which width wilder will woods you your yvonne"
    },
    {
      "section": "tutorial",
      "id": "103_filtering",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope above achieve after alexander alisha allow an and angular app are as asc be being bored break by can case changed class claudine clickheadercell clicking client column columndefs columns company controller core css custom data default desc describe direction disabled editing effect either else emulate enablesorting ethel even event example expectcellvaluematch expected expectheadercellvaluematch expectheadercolumncount external externally false files first firstly foley function functions further gender get got grid grid1 gridapi gridoptions gridtestutils has have header height however html id if ignored ignores illustrate in index indicator internal is it js json key last length main mainctrl manually me module myers name native neal ng-controller ngtouch no not of off on one onregisterapi or original our over overview parsons part picking price property provides removed requested require retrieve return reverse routine routines rows scenario second secondly server should show so sometimes sort sortchanged sortcolumns sorted sorting spec success support suppress switch tells that the their this those three to true turn tutorial two ui ui-grid uigridconstants undefined unsorted useexternalsorting user using values var visible want we when width with woods you your yvonne"
    },
    {
      "section": "tutorial",
