NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "ColumnDef",
      "type": "object",
      "keywords": "$scope active all allowed an and angular-translate any anyway api application apply appropriately are array as available backing bars baz be been before both but by bytes called can cell celleditablecondition cellnav choose chooser chosen class code col coldef column columndef columndefs commonly containing currenttarget default defaults definition dependent determine do does dropdown each edit editablecelltemplate editdropdownfilter editdropdownidlabel editdropdownoptionsarray editdropdownrowentityoptionsarraypath editdropdownvaluelabel editfilechoosercallback editing editor either enable enablecelledit enablecelleditonfocus enabled_ entity evaluated example false falsy feature field file filecontent filename filereader files filter first focus foo for format from function get grid gridcol gridoptions gridrow have having hidden id if ignore ignored in inactive information into invoked is it itself label lastmodifieddate later like list looks may might most name new not object of on one onload only options or passes path populate populated process property read readastext reader receives result return returned row set setfile should size something soon specified status stuff such template text that the then these this to translate true type typically ui ui-grid url use used useful user using value values var we were when which will with would xxx you your"
    },
    {
      "section": "api",
