NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$scope abc acium add adddata and angular app bond brainquil btn btn-success button by can carney changes class click cline columns company comveyer controller copy cox cruz css data data1 data2 describe different element else employed enormo expectheadercolumncount expectrowcount expectrowvaluesmatch false first firstname four fuelton function grid grid1 gridopts gridtestutils have height hopper html id if in index it js kongene kramer lastname length letpro lorraine main mainctrl marcy marqet mclean misty module name nancy new ng-click ng-controller ngtouch oneill origdata1 origdata2 out overview parleynet person pickett providing push reference remove removefirstrow require reset row scenario shepherd should simply spec splice swap swapdata swapping tania the true tutorial type ui ui-grid var visible waters width wise you zamora zomboid"
    },
    {
      "section": "tutorial",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope $timeout access allow almost angular app aquamarine are as attribute background-color basic but by calling can cancel class col colcontainer coldef console controller create css custom data date details done elements expose external-scopes externalscopes fnone for from function functions get getexternalscopes grid gridoptions height html in index isolate js json like log main mainctrl method module more most mynewmodel name new ng-bind ng-click ng-controller ng-repeat ngtouch on one outside overview parseint property reach renderedcolumns return row rowtemplate same scope scopes sec set so start style success sure template that the then to track tutorial ui ui-grid ui-grid-cell up use var variable wait waiting width will with you your"
    },
    {
      "section": "tutorial",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $log $scope abc access alert angular app application attribute btn carney cell celltemplate class columndefs company comveyer controller cox css data employed enormo exists external external-scopes false firstname from fuelton function gender get getexternalscopes give grid gridoptions height html in index is isolate js json lastname lorraine main mainctrl me module mydata myviewmodel name nancy ng-click ng-controller ngtouch no on or overview primary property row scope scopes showme showscope so someprop success template that the then there to true tutorial ui ui-grid use uses using var variables waters width wise within you your"
    },
    {
      "section": "tutorial",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope above achieve after an and angular app are as asc bored break by can case changed class client columndefs company controller core css custom data desc direction editing either else emulate enablesorting event example external externally false files first firstly function functions further gender get got grid gridapi gridoptions has height html if illustrate in index internal is js json length main mainctrl manually me module name native ng-controller ngtouch not of off on one onregisterapi or over overview part picking property provides requested retrieve routine routines rows secondly server show so sometimes sort sortchanged sortcolumns sorted sorting success support suppress switch tells that the their this those three to true turn tutorial two ui ui-grid uigridconstants undefined useexternalsorting user using var want we when width with you your"
    },
    {
      "section": "tutorial",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope $timeout about address age agetemplate all also an angular app appends are btn btn-success button by callspending callspending-- cancel cellnav celltemplate city class click column columndefs company controller css data date describe displayname duplicate edit element email emulates enablecelledit enablecolumnresizing enabled enablefiltering error every example expectvisiblecolumnmenuitems features field for foreach friend friends from function get getrowidentity grid grid1 gridoptions gridtestutils height html id idea index it iter js json left length loading main mainctrl menu module movecolumns mydata name new ng-bind ng-click ng-controller ngtouch not object of on options overview pages pending performance phone pinning push refreshbutton refreshdata require resizecolumns resizeheadercell resizing return row rowidentity rows scenario scope sec seconds selection server setting should spec specifies start state street string success that the this to true tutorial type ui ui-grid ui-grid-cell-contents ui-grid-cellnav ui-grid-edit ui-grid-move-columns ui-grid-pinning ui-grid-resize-columns ui-grid-selection value var watch when width with your zip"
    },
    {
      "section": "tutorial",
