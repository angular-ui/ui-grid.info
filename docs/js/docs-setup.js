NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "Grid",
      "type": "function",
      "keywords": "$parse-able $scope access accessor add added additional addrowheadercolumn adds again all already alter an and any api appended are array as asc ascending assign assigntypes at automatically based be been behavior being block boolean browser build buildcolumndefsfromdata buildcolumns buildstyles by callback callbacks called calls can canvas cell chance change changed class col coldef column columnbuilder columnbuilders columndefinition columndefs columns columnsprocessor completes container contains count created createleftcontainer createrightcontainer creates criteria current currently data datawatchcallback debounced def defined definitions delete deregister deregisterdatachangecallback deregistered desc descending direction directions directive document doesn each either element emits etc event exclude exist existing exists expected false field first flagscrollinghorizontally flagscrollingvertically for from function functional further getcellvalue getcoldef getcolumn getcolumnsorting getgridqualifiedcolfield getrow gets given grid gridcolumn gridoptions gridrow handlewindowresize has hasleftcontainer having header horizontally id identified if in instance into is isrtl isscrollinghorizontally isscrollingvertically it iterate its left leftcontainer load long main maintain map method methods modified modifyrows name needed newrawdata not note notices object objects of on one only optional optionally options or our parameter particular pass per populates position precompilecelltemplates precompiles present process processes processor processrowbuilders promise properties property prototype provided queuerefresh redraw redrawcanvas reference refresh refreshed refreshrows register registercolumnbuilder registercolumnsprocessor registerdatachangecallback registered registerrowbuilder registerrowsprocessor registerstylecomputation remove removerowsprocessor removes render renderable rendered reset resetcolumnsorting resetting resizes resolved resources return returns right rightcontainer righttoleft row rowbuilder rowbuilders rowentity rowequality rows screen scroll scrolling set sets sort sortchanged sortcolumn sorted sorting state stylecomputation supplies tbd templates that the this through to todo triggered true type types ui uid unsorted updated use used uses using value vertically via viewmodel watch when whenever which will window with within you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "$elm a4 accordingly add adjusted and any api are available be being black bold button calling class color columns com containing controls csv current custom default defaults don download element export exporter exporterbuttonlabel exportercsvlinkelement exporterheadertemplate exporterlinklabel exporterlinktemplate exportermenucsv exportermenupdf exporterpdfdefaultstyle exporterpdfmaxgridwidth exporterpdforientation exporterpdfpagesize exporterpdftableheaderstyle exporterpdftablelayout exporterpdftablestyle exportersuppressbutton false feature fixed fontsize for format go grid gridlines gridoptions header https if implying in into it items js landscape layout letter like link mandatory margin match maxium means menu native needed no not null object on or orientation own page paper pdf pdfmake placed portrait present providing resulting roll scaled section selection set should show size sizes style suppressbutton table tableheader tablelayout template text that the their these this to true ui ui-grid use user using usually valid value ve we which width will with you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "$resource $scope about add adds alert an and any api appended application are array as automatically available be before behaviour both box browser by call callback can class column columns commonly concat console consolemessage content context create csv custom data defaults different directly discarded displayname does doesn don each element enabled enableimporter english entity error errorkey errors event false feature field file fileapi first for foreach from function generally given grid gridoptions handle handling have header headerarray headers i18n if import importer importerdataaddcallback importererrorcallback importernewobject importerprocessheaders importershowmenu importing in index inserting internal internationalise internationalised into invalidcsv invalidjson is it item itself javascript jsonnotarray keys knows language log logging logic mandatory match matching may means menu message messages method might mydata myheadercols myheadercolumns myloggingroutine myres myspeciallookupfunction myuserdisplayroutine name names necessarily need new newobjects noheaders non-matching noobjects not null object objects occur of often on one or order otherwise position positioning process processheaders processing property provide provides push rather re recognises required respond return returned returning routine row rowedit rows same save seeks server set should show some source standard support text than that the their therefore these this thiscol through tidier to triggered true typically ui ui-grid use useful user using value vanilla var way we when where whether which will would write written you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "all allow always and api are at available be being can checkbox class conjunction create ctrlkey different enable enablerowheaderselection enablerowselection enableselectall entire false feature for from grid gridapi gridoptions header in is modifierkeystomultiselect multiple multiselect nounselect object of only or prevent requires row rows select selected selection selectionrowheader selectrow set shiftkey single the these to top true ui ui-grid unselect unselected used using when with works you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "GridUtil",
      "type": "service",
      "keywords": "$scope accessor account accounting adapted after alert allow always an and angular any anything apart api app appropriately are argument arguments array as attempt be before bind binding binds bob boolean borders bound bracket browser browsers but by cache calculation called calling calls can capitalized characters column columndefs columnname com comes consistent contents controller converts copied correctly createboundedwrapper current currently data date debounce debounced debouncedfunc decorated decorates deep definitions delay denormalize denormalizes different direction do does dom dommousescroll either element elementheight elementwidth else ends ensures environment etc evaluate event examine excludeproperties execute executed field first firstname following for frank from func function functions get getcolumnsfromdata gettemplate given grid gridutil guesses guesstype handle height html http https id if immediate in inconsistent index inside iow is it item its jones jquery js keep keys last lastname list log logs mainctrl make margin margins matter means method milliseconds modifier module most-recent mouse mousewheel mozmousepixelscroll multiple name names nested newid ng-bind ng-controller ng-model no normalize normalized normalizes normalizescrollleft normalizewheelevent not notation number object of on one only optional or param params path performs pixels points potential preeval promise prop-erty property queues readablecolumnname regardless representing request resolving resulting return returns right rtl scale scrollleft see service set smith so special split stays string sure takes template that the them then this throttle throttled throttledfunc time times to trailing trigger true truthy type ui unique updates url use utility value values var variable wait ways what wheel whether width with wrapper"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope address age agemax agemin aggregation aggregationhidelabel aggregationtype aggregationtypes also an and angular app are avg background-color be below but can class column columndef columndefs columns controller count create css custom customcelltemplate data describe display displayed displayname each enablefiltering enterfilterincolumn enum ever example expect expectfootercellvaluematch expectfootercolumncount expectheadercolumncount field filter footer footercelltemplate footers for function functions get grid grid1 gridoptions gridtestutils have height html id if in index inject is it js json label logic main mainctrl max min module name need ng-controller on option or order overview own pass recalculate red refer require row rows scenario set should show showfooter showfooterrow six spec specific street style success sum supported supports template the then third to total true tutorial ui ui-grid ui-grid-cell-contents uigridconstants use values var visible what which white width will wish with without you your"
    },
    {
      "section": "tutorial",
      "id": "106_binding",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope ability accept adds allows and angular api app are array as assumed at attributes auto-populate be been by can class column columndefs columns company concat controller copy created css csv custom data define defs demand directive displayname doe doing each either elements empty enable enablegridmenu entity example exists extended fasttruck fasttrucks feature female field file files for format formats found from front function gender grid gridapi gridoption gridoptions has heading headings height html if illustrates implemented import imported importer importerdataaddcallback importing imports in include index instead into is it items jane john js json loaded main mainctrl male mandatory many mapped maps match menu module must name native need needing newobjects ng-controller nganimate not objects on once onregisterapi optionally options or other overview picker provide provided received row smith so start system test testicon that the this to true tutorial ui ui-grid ui-grid-importer up use used user uses using var we width will with would you your"
    },
    {
      "section": "tutorial",
      "id": "210_selection",
      ...
      "type": "overview",
      "keywords": "$http $interval $log $scope add added additional address after age all allow allowcellfocus allows always an and angular another api app are at be been box btn btn-success but button by call callback can cannot check checkboxes city class clearall clearselectedrows click clicking cmd-key columndefs controller could css ctrl-key current currently data default demonstrates different digest directive disabled displayname does element enable enablerowheaderselection enablerowselection enableselectall event example examples false first focusable for function gave get getselectallstate grid gridapi gridoption gridoptions has have header height hidden html id if in include index individual individually info is isselected it js json just least log main mainctrl manually may mean means modifierkeystomultiselect module msg multi-select multiple multiselect must name necessary need new ng-bind ng-click ng-controller ng-disabled not note nounselect of on one only onregisterapi open option options or other overview page programatically provide provided register result row rowheader rowheaderselection rows rowselectionchanged scope second secondctrl select selectall selectallrows selectallvisiblerows selected selecting selection selectrow set setmodifierkeystomultiselect setmultiselect setting shift-key shown single so success than that the them then this ticked time to togglemodifierkeystomultiselect togglemultiselect togglerow1 togglerowselection top true tutorial two type ui ui-grid ui-grid-selection unselect upon used uses using var visible wait watch we when whilst width will with without you your"
    },
    {
      "section": "tutorial",
      "id": "211_two_grids",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope all allows and angular api app as at available be behaves below bold bower calling can class color columndefs columns company controller css csv csvexport custom custom-csv-link-location data directive element else example export export_column_type export_format export_row_type exported exporter exporterlinklabel exporterpdfdefaultstyle exporterpdfmaxgridwidth exporterpdforientation exporterpdfpagesize exporterpdftableheaderstyle exporterpdftablestyle exporting false feature field for format found from function gender get grid gridapi gridoptions have height here html if in include index install installed italics js json letter like main mainctrl module must myelement name need ng-click ng-controller ng-model nganimate on onregisterapi options or overview pdf pdfexport pdfmake portrait provide queryselectorall red returned rows selected selection should show success tailor the this through to true tutorial ui ui-grid ui-grid-exporter ui-grid-exporter-csv-link ui-grid-selection use value var visible want way we width will with would you your"
    },
    {
      "section": "tutorial",
      "id": "401_AllFeatures",
