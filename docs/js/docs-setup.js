NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope across address adds after age aggregate aggregatecolumn aggregated aggregateinfo aggregates aggregation aggregations all allow allowing allows alpha also always an and angular any app applying are aren as asc at automatically average avg avoid balance base based be because been being below body breaks btn btn-success but button buttons by calculate calling can cannot cellfilter celltemplate change changed changegrouping changes class cleanly cleargrouping clears code col column columndef columndefs columns columnsprocessor company complex config consideration console controller could count counting counts creating css currency current custom custom_filters customtreeaggregationfinalizerfn customtreeaggregationfinalizertext data deeper default define defining delete dependency desc describe development differently direction distribution do done each easily edited effect either else enablefiltering end enhancement entity example examples excel existing expand expandall expandallrows exported exporter external extracts false feature female files filter filtered filtering finalizer find first follows for foreach format formatting forthcoming from front function functionality gender genderhash genders get getaggregates getgrouping gets go grid grid1 gridapi gridoption gridoptions gridtestutils group groupcolumn grouped groupheader grouping groupingcolumnprocessor groupingindent groupingrowheaderalwaysvisible groupingrowheaderwidth groupingsuppressaggregationtext grouplevel grouppriority grouprowheader groups handled handles has have header height high higher however html id if in include included including inclusion indent indented index individual initialisation input insertion instantiations interact is isn it js json larger laststate least length level leverages like limit list log look looks main mainctrl male mapgender marked match max may means methods min module more move moved moving must name names need ng-click ng-controller ng-if nganimate ngtouch nicer not notable now null number of off on one only onregisterapi option optionally options or order out outstandings overriding overview part particular people permit persisted pinning pivot pixels pleasing potentially present prior priority programmatically provided provides providing proxy rc22 reload remove removes removing rendercontainers rendered replace require result resulting return returned reversed row rowheader rownum rows rtl running runs saved savestate scenario second see selected selection set setgrouping sets setting settings shared should showing shows similar skip skipping slice so some sort sorted space spec special specific standalone start state states statetotal still stop success such suppress suppressremovesort sure table take templates testing text than that the them themselves then there they things this through title to together togglefirstrow togglerow togglerowgroupingstate togglesecondrow tooltip treeaggregation treebase treeindent treelevel treerowheaderalwaysvisible treerowheaderwidth treeview true tuning tutorial type ui ui-grid ui-grid-cell-contents ui-grid-grouping uigridgroupingconstants undefined under ungroupcolumn ungrouped ungroups unit up use used user using usually value values var via visible visiblerowcache visually want wanted was watch way ways we when whether which width will with work working would write writes xxxx xxxxx you your"
    },
    {
      "section": "tutorial",
      "id": "210_selection",
