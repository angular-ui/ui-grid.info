NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope access actually add adding additional additionally adjusts again against age all allows and angular another anything app are area array as back bars be being below binding browser btn btn-success can canvas cell cells changed changes class column columndefs columns combination comes complex complexity component computationally concat consists controller correctly css current currently data degradation determine difference different displayed displaying dom each either elements enablefiltering entire entities every expect expensive fact faked faster featured field filtering flat flatentityaccess for function gender get grid grid1 gridoptions have height html id if illusion in index inside instead into is it js json jumping just large left long main mainctrl makes many means methods middle might module must name needs new ng-click ng-controller nganimate ngtouch not noticable noticeable objects of off on one only operations option or overhead overloading overview particular parts people performance plus point position provide provides provision purely quite rapidly rather remainder render result richly right rows run same saying scenario scroll scrolling see set sets setting show showgridfooter showing shuffles side single smoothly so some sorting space success such support than that the then these think this those tied time to toggle toggleflat true turn tutorial two ui ui-grid uigridconstants use value values var very view virtualises visible way we were weren what whether which whilst white width will winded with without work you your"
    },
    {
      "section": "tutorial",
