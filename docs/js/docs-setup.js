NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "function",
      "keywords": "$delegate $provide $resource $scope add adding adds all allowing allows also altering am an and angular angularjs any api application aquamarine are array as at attribute attributes auto-generating automatically available background-color backwards be becomes behaviour being binding boolean boxes but by can case center class col colcontainer coldef column columndef columndefs columns common compare compared compatibility complex config configure contain content controller could current custom data decorator default defaultoptions defaults define defined developer disable disabled display displayname displays div do documented does doesn each either elements empty enablecolumnmenu enabled enablefiltering enablerowhashing enablesorting entire entitya entityb equality example exclude excludeproperties existing exists factory false field1 field2 file filter filtering first fnone footer footer_template footertemplate for from function generate get getexternalscopes getrowidentity github grid gridoptions grids gridutil has have header header_template headers headertemplate height hideheader hit how html id identifying identity if ignore ignored in include individual information inline integer into inverse is it item large list look maintain may menu method minimum minrowstoshow modify more most mydata name names necessary new nextuid ng-click ng-repeat no not note null number numbers object objects of often on one only option options or out over overlaid overridden performance place point precompiled pretty process properties property provide provided provides providing query rather refer rendered renderedcolumns replace request required resulting return returns row row_template rowequality rowidentity rows rowtemplate second section set setting should show shown sort sorting source sourced speeding standard starting state static string stripped style tailor take template text-align than that the their then they this to tojson track true tutorial ui ui-grid ui-grid-bottom-panel ui-grid-cell ui-grid-header ui-grid-top-panel uigrid uniquely up use used uses using value var vastly visible want when where while widgets will with within x_ you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "GridUtil",
      "type": "service",
      "keywords": "$scope accessor account accounting after alert allow an and angular any apart api app appropriately argument array as be before bob boolean borders bracket browser browsers cache calculation calls can capitalized characters column columndefs columnname com comes consistent contents controller converts copied correctly current currently data date debounce debounced debouncedfunc decorated decorates deep definitions delay denormalize denormalizes different direction dom dommousescroll either element elementheight elementwidth etc evaluate event examine excludeproperties execute executed field first firstname following for frank from func function functions get getcolumnsfromdata gettemplate given grid gridutil guesses guesstype handle height html https id immediate in inconsistent index is it item jones jquery js last lastname list mainctrl make margin margins matter method milliseconds modifier module mouse mousewheel mozmousepixelscroll multiple name names nested newid ng-bind ng-controller ng-model no normalize normalized normalizes normalizescrollleft normalizewheelevent notation number object of on one only optional or path pixels preeval promise prop-erty property readablecolumnname representing resolving resulting return returns right rtl scale scrollleft service set smith so special split stays string sure takes template that the them then this time to type ui unique url use utility value values var variable wait ways what wheel width with"
    },
    {
      "section": "api",
