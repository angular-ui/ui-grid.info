NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "function",
      "keywords": "$delegate $provide $resource $scope add adding adds adjusted all allowing allows also altering am an and angular angularjs any api application aquamarine are array as at attribute attributes auto-generating automatically available background-color backwards be becomes been behaviour being binding boolean boxes but by can case center changes class col colcontainer coldef column columndef columndefs columns common compare compared compatibility complex conditions config configure contain content controller controls could current custom data decorator default defaultoptions defaults define defined developer disable disabled display displayname displays div do documented does doesn each either elements empty enable enablecolumnmenu enabled enablefiltering enablerowhashing enablescrollbars enablesorting entire entitya entityb equality event events example exclude excludeproperties executing existing exists external factory false field1 field2 file filter filterchange filtering fired fires first fnone footer footer_template footertemplate for from function generate get getexternalscopes getrowidentity github grid gridoptions grids gridutil has have header header_template headers headertemplate height hideheader hit horizontal how html id identifying identity if ignore ignored implemented in include individual infinite infinitescrollpercentage information inline integer internal into inverse is it item large list listen logic look maintain may menu method minimum minrowstoshow modified modify more most mydata name names necessary new nextuid ng-click ng-repeat no not note null number numbers object objects of often on one only operate option options or out over overlaid overridden percentage performance place point precompiled pretty prevents process properties property provide provided provides providing query rather refer rendered renderedcolumns replace request requested required resulting return returns row row_template rowequality rowidentity rows rowtemplate scroll scrollbar second section server set setting settings should show shown sort sorter sorting source sourced speeding standard starting state static still string stripped style suppresses tailor take template text-align than that the their then they this to tojson track true tutorial ui ui-grid ui-grid-bottom-panel ui-grid-cell ui-grid-header ui-grid-top-panel uigrid uniquely up updated use used useexternalfiltering useexternalsorting uses using value var vastly vertical visible want what when whenever where which while widgets will with within x_ you your"
    },
    {
      "section": "api",
      ...
      "shortName": "columnDef",
      "type": "object",
      "keywords": "$scope about action active add address adds against aggregation aggregationhidelabel alert algorithm allowing alongside although an and angular any api append apply are array asc auto backward be bind binding blargh boolean by call can cell cellclass cellfilter cellnav celltemplate city class clicked col colrenderindex column columndef columndefs compatibility complex condition configuration contain content context controls custom data date default defaults defining definition definitions desc determine direction disablecolumnmenu disablehiding disables displayed displayname div docs each either element enable enabled enablefiltering enablesorting ends ends_with evaluate evaluates example expression expressions false feature field field1 field2 field3 fields filter filtering filters focus footer for from function functionality generated getfulladdress grid gridoptions gridutil guess guessing guesstype have header headercellfilter headercelltemplate hides hiding icon id if in individual is it item items just knows label like list many maximum maxwidth menu menuitems menus method minimum minwidth multiple must name normal not number object of omitted on one only option options or outer parameters pass percentage placeholder present prevents property provided receive refer removesort return returns rowrenderindex satisfaction scope see service set sets setting settings should show shown single so sort sorting sortingalgorithm source specific specify specifying starts starts_with string supported suppressremovesort takes template term text that the then this title to too translate true tutorial type typically ui ui-grid ui-grid-icon-info-circled uigridconstants use used user using value values visible when whether which widgets width will wish with within work would xxx you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "PublicApi",
      "type": "object",
      "keywords": "$scope active addrowheadercolumn adds all always an and any api array as at back be both building but by calculations call called calls can changed circumstances clearrowinvisible clears code column columns complete completes core criteria currently custom data def else emits end event feature features filter filtering for from function functional functions get getcolumnsorting getting getvisiblerows grid gridapi gridcolumns gridoptions gridrow handlewindowresize handling has have header here if in included instance invisible is it just make manually method more mysortfn necessary no normal normally not note null nulls object obtain of on one onregisterapi or order other otherwise output override parameters passed picked present priority problem promise provides public refresh refreshrows render rendered rendering renderingcomplete resize resolved return returned returns row rowentity rows rowsvisiblechanged same screen set setrowinvisible sets should size so some sort sortchanged sortcolumns sorted sorthandlenulls sorting sorts still stopping that the them then there this time timing to todo trigger ui up used users using value var visibility visible want watch way we were when which will window with within without would you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "gridTest",
      "type": "service",
      "keywords": "about adding all also alternatively although an and any api are array as ascending associated be body box boxes break browser but by can cancel cancelfilterincolumn cancels cell cellvalue cellvalue1 cellvalue3 change changed check checks click clickcolumnmenu clickcolumnmenuremovesort clickcolumnmenusortasc clickcolumnmenusortdesc clickheadercell clicks col colnumber cols column columns constant container could count cumbersome data datacell dataset default descending develop does doing e2etestlibrary each easily element end enter enterfilterincolumn enters error every exists expect expectcellvaluematch expectcolumncount expectedcol expectednumcols expectednumrows expectedrow expectedvalue expectedvaluearray expectfilterboxincolumn expectfootercellvaluematch expectfootercolumncount expectheadercellvaluematch expectheadercolumncount expectheaderleftcolumncount expectitems expectrowcount expectrowvaluesmatch expectvisiblecolumnmenuitems feels fetchcol fetchrow filter filtervalue find finds footer footercell footervalue for fragile from functional functions given go greater grid gridid gridtestutils has have header headercell headervalue helpers how i18n id if in individual inspect internal into is it item items left less ll lots many matches may meaning menu menuitemnumber menus method methods myelement mygrid necessary none not note number numbers numerical of on only option or ordering pinned quite reason recommended regex regexes remove render rendered result return returns row rows script second see service shift-clicks shiftclickheadercell simple small some sort specified standard string strings takes test tests text than that the then there therefore these this to tutorial typically ui ui-grid update updated used useful using usually value values verify virtualisation visible visiblecolumnmenuitems want when whenever which will with within would wrap write you"
    },
    {
      "section": "api",
      "id": "ui.grid.edit",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope accessing action alert alert-warning alexander alisha allows an and angular app app2 application as asc at be below beryl but by can class claudine click clickcolumnmenuremovesort clickcolumnmenusortasc clickcolumnmenusortdesc clickheadercell clicking column columndefs columns company controller core css data def default defining definitions desc describe direction disabled else enable enablesorting ethel example expectcellvaluematch expected expectheadercellvaluematch expectheadercolumncount false feature female field first flag foley for fry function gender get gets grid grid1 grid2 gridapi gridoptions gridtestutils has have header height html id if in include index index2 inital-sort initial is it js json key last level main main2 mainctrl male menu module multiple must myers name neal ng-controller nganimate no not null nulls on onregisterapi options order original overview page parsons price priority property remove require required return reverse rice rows says scenario second see set setting shift shift-click shift-clicking shiftclickheadercell should slide sort sorted sorthandlenulls sorting sortingalgorithm span8 spec state sub-property success that the then third three to true tutorial two ui ui-grid unsorted up values var velma visible want way what which width woods you your yvonne"
    },
    {
      "section": "tutorial",
      "id": "103_filtering",
