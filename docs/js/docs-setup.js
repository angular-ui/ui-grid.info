NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope above accomplished add age alias all allows also an and angular any app are array as asterisks at attribute automatically based basic be below best bishop booleanresult box broken built-in by can cancel cancelfilterincolumn carr cellvalue chosen class clay code column columndefs columns com comes company compare condition conditions conditon contains contents controller create css custom data def default defined defines describe digits disabled do doesn each element elements email enablefiltering ends ends_with enterfilterincolumn even everything example examples except expectcellvaluematch expectfilterboxincolumn expectheadercolumncount expectrowcount fact false field fields filter filtered filtering filters first flag following for function fuzz-match gender get gets greater greater_than grid grid1 gridoptions gridtestutils guess has have height how however html id if in index indexof input inside instead introduced is it javascript js json just less less_than level logic main mainctrl makes male matching may mixers module more multiple mycustomsorter name ng-controller nganimate no noterm now number object occasionally of off on one optionally options or out out-of-the-box overview own page passes phone placeholder pre-populated previous property provide provided re replace require return returns row rows run same scenario search see set setting setup several should show shown signature simple single six spec specifies specify still strip strippedvalue success such supported sure take term terry than that the third this to true tutorial two ui ui-grid uigridconstants use user var variable visible want when where which width wildcard will with within xxx you your"
    },
    {
      "section": "tutorial",
      "id": "104_i18n",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope address age agemax agemin aggregation aggregationhidelabel aggregationtype aggregationtypes also an and angular app are avg background-color be below btn btn-success but can cellfilter class column columndef columndefs columns controller count create css custom customcelltemplate data date dec describe display displayed displayname each enablefiltering enterfilterincolumn enum ever example expect expectfootercellvaluematch expectfootercolumncount expectheadercolumncount field filter footer footerbutton footercellfilter footercelltemplate footers for foreach function functions get grid grid1 gridoptions gridtestutils have height html id if in index inject is it js json label logic main mainctrl max min module name need ng-click ng-controller on option or order overview own parse pass recalculate red refer registered require row rows scenario set should show showfooter showfooterrow six spec specific street style success sum supported supports template the then third to total true tutorial ui ui-grid ui-grid-cell-contents uigridconstants use values var visible what which white width will wish with without you your"
    },
    {
      "section": "tutorial",
      "id": "106_binding",
