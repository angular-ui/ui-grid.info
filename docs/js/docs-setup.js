NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "columnDef",
      "type": "object",
      "keywords": "$scope about action add address adds against alert algorithm allowing although an and angular any api append apply are array asc auto backward be bind binding blargh boolean by can cell cellclass cellfilter cellnav celltemplate city class col colrenderindex column columndef columndefs compatibility complex condition configuration contain content context custom data date default defining definition definitions desc direction displayname div docs each either element enabled enablefiltering enablesorting ends ends_with evaluates example expression expressions false feature field field1 field2 field3 fields filter filtering filters focus for function functionality generated getfulladdress grid gridoptions gridutil guess guessing guesstype have header headercellfilter headercelltemplate icon id if in individual is it items just knows like list many maximum maxwidth menu menuitems minimum minwidth multiple must name normal not number object of omitted on one only options or outer parameters percentage placeholder present property provided receive refer returns rowrenderindex satisfaction scope see service set sets setting should shown single sort sorting sortingalgorithm source specify specifying starts starts_with string takes template term that the then this title to too translate true tutorial type typically ui ui-grid ui-grid-icon-info-circled uigridconstants use used using values visible when whether which widgets width will wish with within work would xxx you your"
    },
    {
      "section": "api",
