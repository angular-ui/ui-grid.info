NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "ColumnDef",
      "type": "object",
      "keywords": "aggregated aggregates aggregation aggregations allowing also an and api apply are as attribute available average be been but calculate calculated calculates called can capable case cases cell celltemplate class coldef column columndef columndefs columns concatenate converted could count creating custom customtreeaggregationfinalizerfn customtreeaggregationfn defaults display displayed do don either else entity false feature fieldvalue filter for form format from fully function getcellvalue grid gridoptions group grouping has header i18n if in information into is it item just label ll manipulate manual most myattribute mycol name native need needs number numbers numvalue object of on or otherwise particular populates provided rather render rendered required result retrieve returning row row-by-row rows run running same set should shouldn so some something squares standard store stored sum than that the them then these this to total totals tree treeaggregation treeaggregationupdateentity treebase treenode treeview true type typeof typically ui ui-grid uigridtreebaseconstants undefined underlying use using value values want wanted was we whatever when which will with won work you"
    },
    {
      "section": "api",
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "api are at available base be but button by children class control create don dynamically each even expand extra false feature for grid gridoptions grouping header icon if incremented indent indents is level load make more need no node nodes number object of overrides pixels planning pleasing programatically provides re row set show showtreeexpandnochildren showtreerowheader single states the there these this to tree treebase treeindent treerowheaderalwaysvisible treerowheaderbasewidth true ui ui-grid use using visually wider width will you youll"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "service",
      "keywords": "achieve actually addorusenode adds adjustsorting after against aggregates aggregation aggregationbase aggregations all allexpanded along already always an and any anything api appropriate are array as associated assumes at back based basing be before being build builder builds built but by calculates called calling calls can cannot check child children col coldef collapse collapseallrows collapsed collapserow collapserowchildren collapses collates column columndef columndefs columns computationally correctly create created createorupdaterowheaderwidth createrowheader creates createtree criteria current data deepest defaults derived descendents determine determines do doing down each effort empty entity events expand expandall expandallrows expanded expandrow expandrowchildren expands expensive exporting false feature filter filtering filters find fix fixfilter flat for from full function get getaggregations given go goes grid gridoptions group grouped grouping has have having header hierarchy how if ignored ignoresort immaterial immediate immediately in information inline into invisible is it itself keep leaves level list lists looks make match may necessary need never no node nodelist nodes normal not now object of on one only operating options or order otherwise our output overwrites parent parents pass passed perform performs point present preserves presumably previous previously process processes processor promise provided putting raise re recorded recursive recursively remembering removes renderable renderablerows rendertree resolved return returns right risk row rowexpandedstates rowheader rowprocessor rows rowsprocessor run runs same see service services set setallnodes setcurrentstate sets setting should showing showtreerowheader since sized so sometimes sort sorted sortignore sorting sorts sorttree specific state subset suppress sure targetstate that the their them then there these they things this those through to toggle togglerowtreestate toggles too tree treebase treebasecolumnbuilder treenode treerowheaderalwaysvisible treerows trees true turns ui unignore update updated updates use used usually ve visible walking walks want was wasting way we were when whether whilst width will with within works"
    },
    {
      "section": "api",
      "id": "ui.grid.treeView",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$filter $http $interval $scope ability address age aggregate aggregation all and angular app asc avg balance callback can cellfilter celltemplate child children class col column columndefs columns columnsprocessor company complex controller count covers create css currency custom custom_filters customtreeaggregationfinalizerfn customtreeaggregationfn data default delete desc describe different direction edit editablecelltemplate editdropdownoptionsarray editdropdownvaluelabel else enabled enablefiltering enablegroupheaderselection entity event false female fieldvalue filter for foreach from function gender genderhash get getrowchildren grid grid1 gridapi gridoptions gridtestutils group groupheader grouping grouppriority groupval header headers height html id if in including index input is isselected js json length main mainctrl male mapgender match max module name ng-controller ng-if nganimate ngtouch null number numvalue of on one onregisterapi other override overview priority re registercolumnsprocessor rendered replace require result return row rowchanged rowheader rows rowselectionchanged scenario select selected selection selectrow setgroupvalues sets setting shown slice so sort spec state success than that the this title to tooltip treeaggregation treebase treelevel treerowheaderalwaysvisible true turn tutorial type typeof ui ui-grid ui-grid-cell-contents ui-grid-edit ui-grid-grouping ui-grid-selection uigridgroupingconstants undefined unselectrow value values var we width with write"
    },
    {
      "section": "tutorial",
      "id": "321_singleFilter",
