NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "PublicApi",
      "type": "object",
      "keywords": "$scope active all always an and any api array as at back be both building but by calculations call called calls can changed circumstances clearrowinvisible clears code columns complete completes core criteria currently custom data else emits end event feature features filter filtering for from function functional functions get getcolumnsorting getting getvisiblerows grid gridapi gridcolumns gridoptions gridrow handlewindowresize handling has have here if in included instance invisible is it just make manually method more mysortfn necessary no normal normally not note null nulls object obtain of on one onregisterapi or order other otherwise output override parameters passed picked present priority problem promise provides public refresh refreshrows render rendered rendering renderingcomplete resize resolved return returned returns row rowentity rows rowsvisiblechanged same screen set setrowinvisible sets should size so some sort sortchanged sortcolumns sorted sorthandlenulls sorting sorts still stopping that the them then there this time timing to todo trigger ui up used users using value var visibility visible want watch way we were when which will window with within without would you your"
    },
    {
      "section": "api",
