NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "service",
      "keywords": "accumulate achieve actual actually added adds after aggregate aggregatecolumn aggregated aggregates aggregation aggregations aggregationtype all also always an and any api are array as asc assumes at back based basing be been beforehand below blank boolean break builder but by call callback called calls can cause change changed changes child children clear cleargrouping clears col coldef collapse collapseallrows collapsed collapserow collapserowchildren collapses column columndef columndefs columnprocessor columns columnsprocessor come config configuration configured consider contains contiguous create created creates current currentgroupheader currently currentvalue data decide default defaults definition descendents described determine do does doesn doing don each effect either end entry existing expand expandallrows expanded expandedstate expandedstatessubset expandrow expandrowchildren expands false feature features field fieldname fields finaliseaggregations finalising find finding first following for format formatting from function get getexpandedstate getgrouping gets given go goes grid gridoptions gridrow group groupcolumn grouped groupfieldstate groupheader groupheaders grouping groupingcolumnbuilder groupingcolumnprocessor groupingheader groupingprocessingstate groupingrowheader grouppriority grouprows groups had handled has hash have header headers held higher how if immediate immediately in including index individual information initialised initialiseprocessingstate inline insertgroupheader inserting insertion into invisible is isn it item items iterate itself just know label left level levels lined link linked ll look maintained maintains make makes matches may method move movegroupcolumns moves name new no node nodes non-group not nothing now object of on once one only onto option options or order other otherwise our out output over parent parents part passed perform perspective portion previous previously priority process processes processing processingstate processor promise prove provided re reasons recognised records recursively reference references reflect refresh relevant remain remove removes renderablerows rendered rendering renumbers reset resetting resolved restore returned right routines row rowexpandedstates rowindex rowprocessor rows rtl rule run runningaggregations same save service services set setallnodes setgrouping sets setting settings setvisibility should showing side single so sort sorting sortpriority specific specifically starting state stateindex still store sub-tree subset such sum text than that the their them then there therefore they this those through tidypriorities to toggle togglerowgroupingstate toggles transient tree triggered type types ui uigridgroupingconstants ungroup ungroupcolumn ungrouped unless up update updated updates upon us use used uses usually value values various ve visibility visible want was we well were when whenever where whether which whilst will with work working works would write writeoutaggregation writeoutaggregations writes writing xxxx you yyyy"
    },
    {
      "section": "api",
      "id": "ui.grid.i18n",
    ...
    },
    {
      ...
      "shortName": "ColumnDef",
      "type": "object",
      "keywords": "aggregated aggregates aggregation aggregations aggregationupdateentity allowing also an and api apply are as attribute available average be been but calculate calculated calculates called can capable case cases cell celltemplate class coldef column columndef columndefs columns concatenate converted could count creating custom customaggregationfinalizerfn customaggregationfn defaults display displayed do don either else entity false feature fieldvalue filter for form format from fully function getcellvalue grid gridoptions group grouping has header i18n if in information into is it item just label ll manipulate manual most myattribute mycol name native need needs number numbers numvalue object of on or otherwise particular populates provided rather render rendered required result retrieve returning row row-by-row rows run running same set should shouldn so some something squares standard store stored sum than that the them then these this to total totals tree treebase treenode treeview true type typically ui ui-grid uigridtreebaseconstants undefined underlying use using value values want wanted was we whatever when which will with won work you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "object",
      "keywords": "api available constant constants grid in module object treebase ui"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope address adds after against age aggregate aggregatecolumn aggregated aggregateinfo aggregates aggregation aggregations aim all allow allowing allows alpha always an and angular any app appropriate are aren as asc at automatically average avg avoid balance base based be because been being below body breaks btn btn-success but button buttons by calculate calling can cannot cellfilter celltemplate change changegrouping class cleanly clear cleargrouping clears code col column columndef columndefs columns company complex config consideration console controller correctly could count counting counts create creating css currency current custom custom_filters data deeper default definitions delete dependency desc describe development direction distribution do does doesn don done each easily edited effect else enablefiltering end enhancement entity example excel existing expand expandall expandallrows exported exporter external extracts fake false feature female fields files filter filtered filtering find first follows for foreach format formatting front function functionality functions gender genderhash genders get getaggregates getgrouping gets go going grid grid1 gridapi gridoption gridoptions gridtestutils group groupcolumn grouped groupheader groupheaders grouping groupingindent groupingrowheaderalwaysvisible groupingrowheaderwidth groupingshowcounts groupingsuppressaggregationtext grouplevel grouppriority grouprowheader groups handle has have header height high however html id if impact in include included including indent indented index indication individual initialisation input insertion interact is isn it js json larger laststate length level leverages like limit list log look main mainctrl male mapgender marked match max means methods min module more moved moving must name names need ng-click ng-controller ng-if nganimate ngtouch nicer no not notable null number objects of off on one only onregisterapi option optionally options or order out outstandings overriding overview part people permanently permit pinning pivot pixels pleasing potentially present priority programmatically provided provides providing proxy refer remove removes removing rendercontainers replace require result resulting return returned reversed row rowheader rownum rows rtl running scenario second see selected selection set setgrouping sets setting settings should shows similar slice so some sort sorted space spec special specific start state states statetotal still stop success suppress suppressremovesort sure table take templates testing text that the them themselves then there they this through title to together togglefirstrow togglerow togglerowgroupingstate togglesecondrow tooltip true turned tutorial type ui ui-grid ui-grid-cell-contents ui-grid-grouping uigridgroupingconstants undefined under ungroupcolumn ungrouped ungroups unit unused up use user using usually value values var visible visiblerowcache visual visually want wanted watch ways we well what when whether which width will with work working would write writes xxxx xxxxx you your"
    },
    {
      "section": "tutorial",
      "id": "210_selection",
