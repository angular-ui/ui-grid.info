NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "api are at available be but class deep each enable enablegrouping entire feature for grid gridoptions group grouping groupingindent groupingrowheaderwidth having header icon if in increase indent indents is level make may more need nested number object of pixels pleasing result row set the these this to too true ui ui-grid using visually wider width you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "service",
      "keywords": "accumulate achieve actual actually adds after aggregate aggregatecolumn aggregated aggregates aggregation aggregations aggregationtype all also always an and any api are array as asc assumes at back based basing be been below blank boolean break builder by called calls can cause change changed changes child children clears col coldef collapse collapseallrows collapsed collapserow collapserowchildren collapses column columndef columndefs columns come configuration configured contains contiguous create created creates current currentgroupheader currently currentvalue data decide default defaults definition descendents described determine do does doesn each effect either end entry existing expand expandallrows expanded expandedstate expandedstatessubset expandrow expandrowchildren expands false features field fieldname fields find finding first following for format from function get getexpandedstate getgrouping gets given goes grid gridoptions gridrow group groupcolumn grouped groupfieldstate groupheader groupheaders grouping groupingcolumnbuilder groupingheader groupingprocessingstate groupmovecolumns grouppriority grouprows groups has hash hasn have header headers held higher how if immediate immediately in including index information initialised initialiseprocessingstate inline insertgroupheader inserting insertion into invisible is isn it item items itself just left level levels lined link linked look maintained maintains make makes matches may method move movegroupcolumns moves name new no node nodes nothing now object of on once one only onto option options or order other otherwise our out output parent perspective portion position previous previously priority process processes processing processingstate processor promise prove provided re reasons recognised records recursively reference references reflect relative relevant remain removes renderablerows renumbers reset resetting resolved retain right row rowexpandedstates rowindex rowprocessor rows rtl rule run runningaggregations service services set setallnodes sets setting settings setvisibility should showing side single so sort sorting sortpriority specific starting state stateindex still store sub-tree subset such than that the their them then there therefore they things this those through tidier tidypriorities to toggle togglerowgroupingstate toggles tree triggered type types ui uigridgroupingconstants ungroup ungroupcolumn unless up update updated updates upon use uses usually value values various visibility visible want was we well were when whenever where which whilst will with work working works write writeoutaggregation writeoutaggregations writes writing xxxx you yyyy"
    },
    {
      "section": "api",
      "id": "ui.grid.i18n",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope able adds after all allows an and angular api app applications are as at automatically available be being bold bom both bower buttons can characters class color columndefs com company contains content controller css csv currentpage custom-csv-link-location data directive do docdefinition don download earlier element enable enablegridmenu enableselectall example excel explorer export exported exporter exportercsvfilename exportercsvlinkelement exporterolderexcelcompatibility exporterpdfcustomformatter exporterpdfdefaultstyle exporterpdffooter exporterpdfheader exporterpdfmaxgridwidth exporterpdforientation exporterpdfpagesize exporterpdftableheaderstyle exporterpdftablestyle exporting false feature few field file first fontsize footerstyle for format found from function gender get getting grid gridapi gridoption gridoptions handful have header headerstyle height html https if import in include index install installed instead internet is issues it italics items js json letter like main mainctrl menu module most must my myfile name native need newer ng-controller nganimate ngtouch not note of on only onregisterapi open opening option options or other overview pagecount pdf pdfmake portrait problem problems queryselectorall rather red repo return rows say selected selection setting show some spreadsheet style styles success successfully tabs text than that the then there this through to tostring true try tutorial ui ui-grid ui-grid-exporter ui-grid-selection use using utf-16 var versions visible want we width will with you your"
    },
    {
      "section": "tutorial",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope above add address age aggregate aggregated aggregation aggregations all allow allowing allows alpha always an and angular any app appropriate are aren as asc at average avg avoid balance based be because being below body breaks btn btn-success but button buttons by calculate can cannot capability cellfilter change class cleanly clear column columndef columndefs columns company complex consideration controller correctly count counting create creating css currency data deep deeper default definitions dependency desc describe development direction distribution does doesn don done each easily edited effect enablefiltering enhancement entity example excel expand expandall expandallrows exported exporter external fake feature fields files filtered filtering find first for format formatting front function functionality functions gender get gets going grid grid1 gridapi gridoptions gridtestutils group grouped groupheader grouping groupingindent groupingrowheaderwidth groupingsuppressaggregationtext grouppriority handle has have header height high however html id if important in include included indent indented index indication insertion is it js json larger length level leverages limit list look main mainctrl make marked max mean means min module more moved moving must name names need needed ng-click ng-controller nganimate ngtouch nicer not notable notify number objects of on only onregisterapi option optionally options or order out outstandings overview part people permit pinning pivot pixels pleasing present priority probably programmatically provides providing proxy re refer rendercontainers replace require resulting returned reversed row rownum rows rtl running savestate scenario second see selected selection set setting should similar slice so some sort sorted spec special specific start state still success suppress suppressremovesort sure table testing text that the themselves then they this through to together togglefirstrow togglerow togglerowgroupingstate togglesecondrow true tutorial type ui ui-grid ui-grid-grouping ui-grid-pinning uigridgroupingconstants under ungrouped unit use user using usually value values var visiblerowcache visual visually want watch ways we well what when whether which wider width with work xxxx you your"
    },
    {
      "section": "tutorial",
      "id": "210_selection",
