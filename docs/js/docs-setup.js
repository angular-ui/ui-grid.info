NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "always and api are at available be but class come counts deep each enable enablegrouping entire even false feature for forces go grid gridoptions group grouped groupheader grouping groupingindent groupingrowheaderalwaysvisible groupingrowheaderwidth groupingshowcounts grouprowheader having header icon if in increase indent indents is level make may more need nested nothing number object of on pixels pleasing preferable present result row rows set shows situations some the these this to too true ui ui-grid using visually wider width you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$compile $http $rootscope $scope $timeout after and angular animates animating app as available because block body bootstrap btn btn-primary btn-success button buttonclose by call calling can cases class click close columns controller core correct css data display element elm enersol ethel expected expectheadercolumncount expectrowvaluesmatch factory female find finishes function get grid grid1 gridapi gridoptions gridtestutils handlewindowresize height hidegrid html id if in index initial is isn it js json main mainctrl may modal modal-body modal-content modal-dialog modal-footer modal-header modalstyle module mymodal name necessary new ng-click ng-controller ng-style ngtouch on only onregisterapi open opening overview particular popup prepend price remove render renders require resize return scenario should show showbutton showmodal smaller some space spec still success than that the this three true tutorial ui ui-grid using usually var whilst width with you your"
    },
    {
      "section": "tutorial",
      "id": "111_cellClass",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope an and angular app asc background-color be blue but can case cell celltooltip class classes clickheadercell col color coloring colors column columndef columndefs columns company contents controller core create css current data datachange derived describe description displayable enablesorting entity every example expect expectheadercolumncount extra field fields for foreground from function get getcssvalue grid grid1 gridapi gridoptions gridtestutils has have header headercell height highlight hovers html id impact in index is it js json likely long main mainctrl module name ng-controller nganimate ngtouch no normal not note notifydatachange on only onregisterapi or over overall overview per performance pop rather recommended red require return returns rgba row scenario set should simply so sort sortchanged spec starts success that the them this title to toequal tooltip tooltips true turn turning tutorial two ui ui-grid uigridconstants up user value var visible watcher when which width will with within won yellow you"
    },
    {
      "section": "tutorial",
      "id": "120_RTL",
