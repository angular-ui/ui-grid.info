NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "service",
      "keywords": "add adds addtomenu adjust align alignment all allow allowing an and any api applied appropriate are around array as at attributes base based basic be before bom browser by calculatepdfheaderwidths call casting cellfilters cells checks client colleagues coltypes column columns com consumable content continues continuing contribute csv csv-ified csvcontent csvexport current data definition dependency determines different displayname document don download downloadfile drawn drawnwidth each everything export exportcolumnheaders exportdata exported exporter exporteralldatafn exporterenableexporting exporterolderexcelcompatibility exportheaders exports extra false feature field file filename for format formatascsv formatfieldascsv formatrowascsv formatrowaspdf formats formatted from function generation getcolumnheaders getdata gets given got grid gridwidth has have header headercellfilters headers heuristic his https ie if immediately in including information installed into is isie issue it items like load loadalldataifneeded logic mainly marked may maybe meaning menu must name need new not note numeric object objects of on only opens options or our overall pagination pass pdf pdf-able pdfexport pdfmake plus processing promise provided put quotes ready renderaspdf renders resolved resulting return returns row rows rowtypes select selected sends server service services should side single so string string-ified strings take that the them then they this title to triggers turned type ui uigridexporterconstants use used user using utf-16 valid value values version visible we when where whether which width widths will window with"
    },
    {
      "section": "api",
