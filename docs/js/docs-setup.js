NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "GridOptions",
      "type": "object",
      "keywords": "$elm $translate a4 accordingly add adding adjusted alignment allowing allows along already an and angular-translate any api applied apply are array as automatically available be before being black bold but button buttons by call callback called calling can cannot case cellvalue changes character class codes col color column columns com comes complete complex containing content context controls convert csv current currentpage custom data date decodes decodestatus default defaults definition display displayname displaynames docdefinition document don download each element example export exported exporter exportercsvcolumnseparator exportercsvlinkelement exporterfieldcallback exporterheaderfilter exporterheadertemplate exporterlinklabel exporterlinktemplate exportermenucsv exportermenulabel exportermenupdf exporterpdfcustomformatter exporterpdfdefaultstyle exporterpdffooter exporterpdfheader exporterpdfmaxgridwidth exporterpdforientation exporterpdfpagesize exporterpdftableheaderstyle exporterpdftablelayout exporterpdftablestyle exportersuppresscolumns exportersuppressmenu exporting exports false feature field filters fixed follow fontsize footer footerstyle for format from function go grid gridcol gridlines gridoptions gridrow had have header https if implying in instant internationalisation into is it items js landscape layout left letter like lines link list mandatory margin massaged massaging match maxium means menu method more must my name names native need needed needs no not note null numbers object of on once or orientation other our output own page pagecount paper part passed pdf pdfmake placed portrait present promise provide provides providing raw ready require resulting return returned returns right roll routine row scaled section selection selectionrowheader separator set should shouldn show shown simple size sizes so some specific status style styles styling supported suppressbutton suppressed synchronous system table tableheader tablelayout takes template text that the their then these this to tostring true ui ui-grid updated use useful user using usually valid value ve we were which width will with you your"
    },
    {
      "section": "api",
