NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "columnDef",
      "type": "object",
      "keywords": "$scope action add address adds against alert algorithm allowcellfocus allowed allowing although an and angular any api are array as asc auto backward be before bind binding blargh boolean both but by can cell celleditablecondition cellnav city class col coldef column columndef columndefs compatibility complex configuration constant context data default definition definitions desc determine direction displayname docs does each edit editablecelltemplate editing editor either element enable enablecelledit enablecelleditonfocus enablecolumnresizing enabled enabled_ enablefiltering enablepinning enablesorting entity evaluated evaluates example expression expressions false falsy feature field field1 field2 field3 filter filtering filters focus for function functionality getfulladdress grid gridoptions have header how icon id if in individual insert invoked is it items just like many maximum maxwidth menu menuitems minimum minwidth must name nav normal not number object of omitted on one options or outer parameters percentage pinning present property provided receives refer resizecolumns resizing return row scope see set sets setting should something soon sort sorting sortingalgorithm source specified takes template term text that the then this title to too true tutorial typically ui ui-grid-icon-info-circled uigridconstants unclear url use used value values visible what when whether which widgets width wish with within work would xxx you"
    },
    {
      "section": "api",
