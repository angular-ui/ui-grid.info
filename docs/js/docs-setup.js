NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$timeout able accomodate achieve adds again all allow allowing already also an and another any api appear appears apply are as at available availablewidth based be because before builders buildstyles but by calc calcheaders calculate calculates calculations call called canvas cases change changed changes code column columnfooterheight columns columnsprocessors columnwidth complete complicated computation consolidate container containers conversely core correct correctly could creates current currently cycle debounce decides declared defined dependencies determine determines determining deterministic digest do does doesn each element elements etc example except execute executed exists expect extra filtering filters first fixed focused footerheight for from full function functions gap getfooterstyles grid gridfooterheight gridrendercontainer grouping has have having header headerheight headers height heights if in include including individually inline involved is isn it iteration iterative just key largely layout left like main make maximum may method methods more most multiple must need needs no non-body not number of on once only options or order ordering other overall overview particular pass percentage pinned pinning preference probably process processors promise prototype question queuegridrefresh re re-rendering really redrawinplace redraws reference refresh refreshcanvas refreshes refreshrows remove render rendercontainer rendercontainers rendered rendering resizes resolves resolving responsive return rows rowsprocessors run said same scroll seems sets settings setvisiblecolumns setvisiblerows should similar single size sized sizing some sorting source stage style styles such telling that the then there therefore they things thinking this those through timeout to uigrid uigridrendercontainer update updatecolumnwidths updates use values viewport visibility visible vision wait want way we what when whenever which why width widths with without wouldn you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "Grid",
      "type": "function",
      "keywords": "$parent $parse-able $scope access accessor according add added additional addrowheadercolumn adds all already alter always an and any api appended application appscope appscopeprovider are array as asc ascending assign assigned assignment assigntypes at automatically based be been behavior being block boolean browser build buildcolumndefsfromdata buildcolumns building builds buildstyles by callback callbacks calldatachangecallbacks called calls can cannot canvas cell chain chance change changed class col coldef column columnbuilder columnbuilders columndef columndefinition columndefs columnprocessor columnrefreshcallback columns completes considered container containers contains context controller count created createleftcontainer createrightcontainer creates criteria current currently data datachange datachangecallback debounced debouncing declared def default defaults defined definitions deregister desc descending direction directions directive doesn down each edit either element emits etc event exclude exist existing exists expected false field fires first flag flags flagscrollinghorizontally flagscrollingvertically for from function functional further getcellvalue getcoldef getcolumn getcolumnsorting getgridqualifiedcolfield getrow gets given grid gridcolumn gridoptions gridrow handlewindowresize handling has hasleftcontainer hasleftcontainercolumns hasrightcontainercolumns having header height horizontally id identified if in indicate inform informed instance intended into is isrtl isscrollinghorizontally isscrollingvertically issue it iterate its js left leftcontainer list load long main maintain map matching method methods might modified modifyrows much must name needed newrawdata next none not note notices notified notifies notifydatachange number object objects occurred occurs of on one only optional optionally options or order other otherwise our override parameter parent particular pass passed per percentage pick populates position precompilecelltemplates precompiles present process processes processor processorfunction processors processrowbuilders processrowscallback promise properties property prototype provided public queuegridrefresh queuerefresh queues recalculated recalculates redraw redrawcanvas reference refresh refreshcanvas refreshed refreshes refreshing refreshrows register registercolumnbuilder registercolumnsprocessor registerdatachangecallback registered registerrowbuilder registerrowsprocessor registerstylecomputation remove removed removerowsprocessor removes render rendered renderedcolumnstoprocess reorder reset resetcolumnsorting resetting resizes resolved resources return returns right rightcontainer righttoleft row rowbuilder rowbuilders rowentity rowequality rows rules run scope screen scroll scrolldirection scrolling set sets sizing some sort sortchanged sortcolumn sorted sorting specifically specified state stylecomputation styles supplies tbd tell tells templates that the their they this through to triggered triggers true type types ui ui-grid uigridconstants unsorted up update updatecanvasheight updated us use used users uses using value values ve vertically via viewmodel visible want watch watches way we when whenever which will window with within you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "PublicApi",
      "type": "object",
      "keywords": "$scope acting active add added addrowheadercolumn adds addtogridmenu adjust advantage again all allowing also always an and any api are aren array as assumes at back based basically be better both building but by calculations call called calling calls can cases cell change changed circumstances classes clearrowinvisible clears code column columndef columnprocessors columns columnvisibilitychanged complete completes config consider core criteria currently custom cycle data datachange debounced def described desired digest directly do does during edit else emits enabled end event execute execution feature features field filter filtering fire for format found from function functions general get getcolumnsorting getting getvisiblerows grid gridapi gridcolumns gridmenuscope gridoptions gridrow handle handlewindowresize handling has have header headercellclasses here id if in included instance instead invisible is isn it item items itself just last like made make management manually menu method might more most multiple must mysortfn necessary no normal normally not note nothing noticing notify notifydatachange null nulls object obtain occurred of on one only onregisterapi or order other otherwise out output override parameter parameters particularly passed picked prefer present priority problem promise provide provided provides public queuegridrefresh queuerefresh re-evaluated re-runs redrawinplace refresh refreshcanvas refreshes refreshrows regenerate registeredmenuitems relevant remain remove removefromgridmenu removes render rendered rendering renderingcomplete request requests resize resolved result return returned returns row rowentity rowprocessors rows rowsvisiblechanged run runs same screen set setrowinvisible sets should shown silently simplify size sizing so some something sort sortchanged sortcolumns sorted sorthandlenulls sorting sorts specified specify still stopping tells that the their them then there they this those time timing to todo trigger tutorial type ui uigridconstants unique up update updates us use used users using value values var ve version visibility visible want was watch way we well were when where which will window with within without would you your"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "shortName": "PublicApi",
      "type": "object",
      "keywords": "again all api be call children collapse collapseallrows collapsed collapserow expand expandallrows expanded expandrow expandrowchildren expands feature for function grandchildren grid grouped grouping if immediate is it object of public row rows specified the to toggle togglerowgroupingstate ui when will wish you"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "service",
      "keywords": "accumulate achieve actual actually adds after aggregate aggregatecolumn aggregated aggregates aggregation aggregations aggregationtype all also always an and any api are array as asc assumes at back based basing be below blank boolean break builder but by called calls can cause change changed changes child children clears col coldef collapse collapseallrows collapsed collapserow collapserowchildren collapses column columndef columndefs columnprocessor columns columnsprocessor come configuration configured consider contains contiguous create created creates current currentgroupheader currently currentvalue data decide default defaults definition descendents described determine do does doesn doing don each effect either end entry existing expand expandallrows expanded expandedstate expandedstatessubset expandrow expandrowchildren expands false features field fieldname fields find finding first following for format from function get getexpandedstate getgrouping gets given go goes grid gridoptions gridrow group groupcolumn grouped groupfieldstate groupheader groupheaders grouping groupingcolumnbuilder groupingcolumnprocessor groupingheader groupingprocessingstate groupingrowheader grouppriority grouprows groups handled has hash have header headers held higher how if immediate immediately in including index information initialised initialiseprocessingstate inline insertgroupheader inserting insertion into invisible is isn it item items itself just left level levels lined link linked ll look maintained maintains make makes matches may method move movegroupcolumns moves name new no node nodes non-group not nothing now object of on once one only onto option options or order other otherwise our out output parent part passed perspective portion previous previously priority process processes processing processingstate processor promise prove provided re reasons recognised records recursively reference references reflect refresh relevant remain removes renderablerows rendering renumbers reset resetting resolved right row rowexpandedstates rowindex rowprocessor rows rtl rule run runningaggregations service services set setallnodes sets setting settings setvisibility should showing side single so sort sorting sortpriority specific starting state stateindex still store sub-tree subset such than that the their them then there therefore they this those through tidypriorities to toggle togglerowgroupingstate toggles transient tree triggered type types ui uigridgroupingconstants ungroup ungroupcolumn ungrouped unless up update updated updates upon us use uses usually value values various visibility visible want was we well were when whenever where whether which whilst will with work working works write writeoutaggregation writeoutaggregations writes writing xxxx you yyyy"
    },
    {
      "section": "api",
      "id": "ui.grid.i18n",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $scope above accomplished actually add adddates age alias all allows also an and angular any app are array as asterisks at attribute automatically based basic be below best bishop booleanresult box broken btn btn-success built-in but by can cancel cancelfilterincolumn carr cellfilter cellvalue chooser chosen class clay code column columndefs columns com comes company compare condition conditions conditon contains contents controller core create css custom data datachange date dates def default defined defines describe digirang digits disabled do doesn each element elements email enablefiltering ends ends_with enterfilterincolumn even every everything example examples except expectcellvaluematch expectfilterboxincolumn expectheadercolumncount expectrowcount fact false field fields filter filtered filtering filters first flag following for foreach function fuzz-match gender get getdate gets greater greater_than grid grid1 gridapi gridoptions gridtestutils guess has hatfield have height how however html hudson id if implement in includes index indexof input inside instead introduced is isn it javascript js json just less less_than level logic main mainctrl makes male match matching may mixeddate module more multiple mycustomsorter name need new nextweek ng-click ng-controller nganimate ngtouch no noterm notifydatachange now number object occasionally of off on one onregisterapi optionally options or out out-of-the-box overview own page passes phone placeholder pre-populated previous property provide provided re replace require requires return returns row rows run same scenario search see set setdate setting setup several should show shown signature simple single six so spec specifies specify still strip strippedvalue success such supported sure take term terry than that the their then there these third this to today togglefiltering true tutorial two ui ui-grid uigridconstants use user var variable visible want way when where which widget width wildcard will with within work xxx you your"
    },
    {
      "section": "tutorial",
      "id": "104_i18n",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope above add address against age aggregate aggregated aggregation aggregations aim all allow allowing allows alpha always an and angular any app appropriate are aren as asc at average avg avoid balance based be because being below body breaks btn btn-success but button buttons by calculate can cannot capability cellfilter change class cleanly clear column columndef columndefs columns company complex consideration controller correctly count counting counts create creating css currency data deep deeper default definitions dependency desc describe development direction distribution does doesn don done each easily edited effect enablefiltering enhancement entity example excel expand expandall expandallrows exported exporter external fake false feature fields files filtered filtering find first for format formatting front function functionality functions gender get gets going grid grid1 gridapi gridoption gridoptions gridtestutils group grouped groupheader groupheaders grouping groupingindent groupingrowheaderalwaysvisible groupingrowheaderwidth groupingshowcounts groupingsuppressaggregationtext grouppriority grouprowheader handle has have header height high however html id if impact important in include included indent indented index indication insertion is it js json larger length level leverages like limit list look main mainctrl make marked max mean means min module more moved moving must name names need needed ng-click ng-controller nganimate ngtouch nicer no not notable notify number objects of on one only onregisterapi option optionally options or order out outstandings overview part people permanently permit pinning pivot pixels pleasing present priority probably programmatically provides providing proxy re refer rendercontainers replace require resulting returned reversed row rowheader rownum rows rtl running savestate scenario second see selected selection set setting should similar slice so some sort sorted spec special specific start state still success suppress suppressremovesort sure table testing text that the themselves then they this through to together togglefirstrow togglerow togglerowgroupingstate togglesecondrow true turned tutorial type ui ui-grid ui-grid-grouping ui-grid-pinning uigridgroupingconstants under ungrouped unit unused use user using usually value values var visible visiblerowcache visual visually want watch ways we well what when whether which wider width with work xxxx you your"
    },
    {
      "section": "tutorial",
      "id": "210_selection",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$http $interval $scope $timeout above access achieve add all allows almost also and angular any app appscope aquamarine are as at background-color basic been body but buttons by can cancel cases cell celltemplate changes class classes code col colcontainer coldef column columndefs com company console continues controller couldn create css cumulative cumulativetotal cumulativewidgets custom customisations data date desirable details different do done elements enablefiltering entity example false features feel field filtered fnone for foreach from function functionality functions gender generally get good grid gridoptions has have headercelltemplate height html https if in including index inject is isn it items js json keep least like log look main mainctrl make many may mean modified module more most myrow myrowfound name need needed new ng-bind ng-click ng-controller ng-repeat ngtouch of on one or other others outside override overriding overview parseint point practice range remember rendercontainers renderedcolumns return review row rowtemplate same scope sec settings sorted standard start starting style success sure template templates that the there this through title to tooltip top totals track true tutorial typically ui ui-grid ui-grid-cell ui-grid-cell-contents upgrade use var very visiblerowcache wait waiting we when which widgets width with work would write you your"
    },
    {
      "section": "tutorial",
      "id": "401_AllFeatures",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$scope achieve actual adding against aims all an and android angular answer any apparently are aren as asked automatically bad be body browser browsers but by can celltemplate change characters chinese chrome class column columns common compatible compliant content could counter cover current currently data depends display displayed displayname do doesn dynamically each easy enabled expect explicit faq field filtered filters firefox fonts foreach format forward frequently full function functionality generally get give given go gotchas grid grids hasn have height here hidden high how html5 id ie9 if in index indexof installation installed intend internal internals is it javascript just know korean latter like list means most must my mydata name names need ng-show nor number numbers of ok on one opera or original our overview page particularly properly question questions re really refer remain render rendercontainers renders resize responsive row rows safari same sequence sequential set short show someone something sorted sorts stands supported take testing that the them then there things this to trying turned tutorial two ui ui-grid-cell-contents unique until up use useless uses using verifies versions virtualisation visiblerowcache want we what when whenever which will with within would you your"
    }
  ],
  "apis": {
    "api": true,
