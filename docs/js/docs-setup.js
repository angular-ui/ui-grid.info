NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "PublicApi",
      "type": "object",
      "keywords": "$scope acting active add added addrowheadercolumn adds addtogridmenu adjust advantage after again all allowing already also always an and any api are aren array as assumes at back based basically be better both building but by calculations call called calling calls can case cases cell certain change changed circumstances classes clearrowinvisible clears code coldef column columndef columnprocessors columns columnvisibilitychanged combo complete completely completes config consider core criteria currently custom cycle data datachange debounced def described desired digest directly do does during edit else emits enabled end event execute execution feature features field filter filtering finished fire for format found from function functions general get getcolumnsorting getting getvisiblerows grid gridapi gridcol gridcolumns gridmenuscope gridoptions gridrow handle handlewindowresize handling has have header headercellclasses here id if in included instance instead invisible is isn it item items itself just last like made make management manually menu method might more most multiple must mysortfn necessary no normal normally not note nothing noticing notify notifydatachange null nulls object obtain occurred of on one only onregisterapi or order other otherwise out output override parameter parameters particularly passed picked prefer present priority problem promise provide provided provides public queuegridrefresh queuerefresh re-evaluated re-runs redrawinplace refresh refreshcanvas refreshes refreshrows regenerate registeredmenuitems relevant remain remove removefromgridmenu removes render rendered rendering renderingcomplete request requests resize resolved result return returned returns row rowentity rowprocessors rows rowsvisiblechanged run runs same screen scroll scrolling scrolls scrollto scrolltoifnecessary set setrowinvisible sets should shown silently simplify size sizing so some something sort sortchanged sortcolumns sorted sorthandlenulls sorting sorts specified specify still stopping such tells that the their them then there they this those time timing to todo trigger tutorial type ui uigridconstants unique up update updates us use used users using value values var ve version view visibility visible want was watch way we well were when where which will window with within without would you your"
    },
    {
      "section": "api",
